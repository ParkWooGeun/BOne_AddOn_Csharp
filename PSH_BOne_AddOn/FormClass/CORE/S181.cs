...
namespace PSH_BOne_AddOn.Core
{
	...
	/// AP대변메모
	/// </summary>
	internal class S181 : PSH_BaseClass
	{
		...
		public override void LoadForm(string formUID)
		{
			try
			{
				...
				SubMain.Add_Forms(this, formUID, "S181");
				S181_CreateItems();
                S181_EnableFormItem(false);
            }
			catch (Exception ex)
			{
        ...
        private void S181_CreateItems()
        {
            ...
            try
            {
                ...
                oNewITEM.Specific.DataBind.SetBound(true, "ORPC", "U_TradeType");
        ...
        private void S181_EnableFormItem(bool Status)
        {
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        private bool S181_CheckDataValid()
        {
                ...
                else if (pVal.BeforeAction == false)
                {
                    if (pVal.ItemUID == "1")
                    {
                        if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                        {
                            ...
                            sQry = "SELECT MAX(DocEntry) AS DocEntry FROM ORPC";
                            oRecordSet.DoQuery(sQry);
                            DocEntry = oRecordSet.Fields.Item(0).Value.ToString().Trim();

                            sQry = "Exec S181_01 '" + DocEntry + "'"; //검수입고문서 취소 프로시저 
                            oRecordSet.DoQuery(sQry);
                        }
                        else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                        {
