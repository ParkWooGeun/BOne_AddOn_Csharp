...
namespace PSH_BOne_AddOn.Core
{
	...
	/// </summary>
	/// 
	internal class S426 : PSH_BaseClass
	{
		/// <summary>
		/// Form 호출
		/// </summary>
		...
		public override void LoadForm(string formUID)
		{
			try
			{
				...
				oForm.Freeze(true);
				SubMain.Add_Forms(this, formUID, "S426");
				PS_S426_CreateItems();
		...
		/// S426_CreateItems
		/// </summary>
		private void PS_S426_CreateItems()
		{
			...
			try
			{
				...
				newItem.Specific.DataBind.SetBound(true, "OVPM", "U_BPLId");
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "5" | pVal.ItemUID == "32")
						{
							...
							sQry = "Select VATRegNum From OCRD Where CardCode = '" + oForm.Items.Item("5").Specific.Value.ToString().Trim() + "'";
							oRecordSet.DoQuery(sQry);

							oForm.Items.Item("26").Specific.VALUE = oRecordSet.Fields.Item(0).Value.ToString().Trim();
							oForm.Items.Item("5").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
						}
					}
				}
