...
namespace PSH_BOne_AddOn
{
	...
	/// 계정별보조원장
	/// </summary>
	internal class PS_FI100 : PSH_BaseClass
    {
		...
		public override void LoadForm(string oFormDocEntry01)
		{
			...
			try
			{
				oXmlDoc01.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_FI100.srf");
				...
				oFormUniqueID01 = "PS_FI100_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID01, "PS_FI100");                   // 폼추가
				...
				PSH_Globals.ExecuteEventFilter(typeof(PS_FI100));
			}
			catch (Exception ex)
			{
		...
		private bool HeaderSpaceLineDel()
		{
			...
			catch (Exception ex)
			{
				if (ErrNum == 1)
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("기간 시작일은 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else if (ErrNum == 2)
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("기간 종료일은 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else
				{
					PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
		...
		private void Print_Query()
		{
			...
			try
			{
				...
				// Crystal 
				WinTitle = "[PS_FI100] 계정별보조원장";
				ReportName = "PS_FI100_01.RPT";
