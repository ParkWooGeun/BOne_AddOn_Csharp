...
namespace PSH_BOne_AddOn
{
	...
	/// 원가요소(배부담당)별비용집계대장
	/// </summary>
	internal class PS_FI210 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry01)
		{
			...
			try
			{
				oXmlDoc01.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_FI210.srf");
				...
				oFormUniqueID01 = "PS_FI210_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID01, "PS_FI210");                   // 폼추가
				...
				PSH_Globals.ExecuteEventFilter(typeof(PS_FI210));
			}
			catch (Exception ex)
			{
		...
		private void CreateItems()
		{
			...
			try
			{
				...
				optBtn = oForm.Items.Item("OptBtn01").Specific;
				optBtn.ValOn = "1";
				optBtn.ValOff = "0";
				...
				optBtn = oForm.Items.Item("OptBtn02").Specific;
				optBtn.ValOn = "2";
				optBtn.ValOff = "0";
				optBtn.DataBind.SetBound(true, "", "OptionDS");
		...
		private void ComboBox_Setting()
		{
			...
			try
			{
				...
				oForm.Items.Item("PrcCode").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
		...
		private bool HeaderSpaceLineDel()
		{
			...
			try
			{
				...
				if (string.IsNullOrEmpty(oForm.Items.Item("EAcctCode").Specific.Value.ToString().Trim()))
				{
					ErrNum = 3;
					throw new Exception();
				}
			...
			catch (Exception ex)
			{
				...
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("시작계정코드는 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else if (ErrNum == 3)
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("종료계정코드는 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else
				{
					PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
		...
		private void Print_Query()
		{
			...
			try
			{
				...
				PrcCode = oForm.Items.Item("PrcCode").Specific.Selected.Value.ToString().Trim();
				...
				WinTitle = "[PS_FI210] 원가요소(배부담당)별비용집계대장";
				ReportName = "PS_FI210_01.RPT";
				...
				if (BPLId == "0")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@BPLId", "전체"));
				}
				else
				{
				...
				dataPackParameter.Add(new PSH_DataPackClass("@ym", YYYYMM));
				...
				dataPackParameter.Add(new PSH_DataPackClass("@PrcCode", PrcCode));
				dataPackParameter.Add(new PSH_DataPackClass("@dc", OptBtnValue));
