...
namespace PSH_BOne_AddOn
{
	...
	/// 차입금대장   PS_FI933
	/// </summary>
	internal class PS_FI933 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry01)
		{
			...
			try
			{
				oXmlDoc01.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_FI933.srf");
				...
				oFormUniqueID01 = "PS_FI933_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID01, "PS_FI933");                   // 폼추가
				...
				PSH_Globals.ExecuteEventFilter(typeof(PS_FI933));
			}
			catch (Exception ex)
			{
		...
		private bool HeaderSpaceLineDel()
		{
			...
			catch (Exception ex)
			{
				if (ErrNum == 1)
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("차입일자의 시작일자는 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else if (ErrNum == 2)
				{
					PSH_Globals.SBO_Application.StatusBar.SetText("차입일자의 종료일자는 필수사항입니다. 확인하여 주십시오.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
				}
				else
				{
					PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
