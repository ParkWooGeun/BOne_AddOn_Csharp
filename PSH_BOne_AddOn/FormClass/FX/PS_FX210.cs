...
namespace PSH_BOne_AddOn
{
	...
	/// 고정자산이력현황
	/// </summary>
	internal class PS_FX210 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_FX210.srf");
				...
				oFormUniqueID = "PS_FX210_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_FX210");
				...
				oForm.Freeze(true);
				PS_FX210_CreateItems();
				PS_FX210_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_FX210_CreateItems
		/// </summary>
		private void PS_FX210_CreateItems()
		{
			try
			{
				...
				oForm.DataSources.UserDataSources.Add("HisType", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 100);

				oForm.Items.Item("BPLId").Specific.DataBind.SetBound(true, "", "BPLId");
				oForm.Items.Item("DocDateFr").Specific.DataBind.SetBound(true, "", "DocDateFr");
				oForm.Items.Item("DocDateTo").Specific.DataBind.SetBound(true, "", "DocDateTo");
				oForm.Items.Item("HisType").Specific.DataBind.SetBound(true, "", "HisType");

				oForm.Items.Item("DocDateFr").Specific.VALUE = DateTime.Now.ToString("yyyy") + "0101";
		...
		/// PS_FX210_ComboBox_Setting
		/// </summary>
		private void PS_FX210_ComboBox_Setting()
		{
		...
		/// <summary>
		/// PS_FX210_DataValidCheck
		/// </summary>
		/// <returns></returns>
		private bool PS_FX210_DataValidCheck()
		{
		...
		/// <summary>
		/// PS_FX210_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_FX210_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string BPLId;
			...
			try
			{
				...
				HisType = oForm.Items.Item("HisType").Specific.Value.ToString().Trim();

				if (string.IsNullOrEmpty(HisType))
                {
					HisType = "%";
				}
				...
				dataPackParameter.Add(new PSH_DataPackClass("@HisType", HisType));
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Btn01")
					{
						...
						{
							if (PS_FX210_DataValidCheck() == false)
							{
								BubbleEvent = false;
								return;
							}
						...
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_FX210_Print_Report01);
