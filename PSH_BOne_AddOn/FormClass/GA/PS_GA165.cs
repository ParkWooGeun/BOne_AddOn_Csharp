...
//****************************************************************************************************************
		...
		public SAPbouiCOM.Matrix oMat01;
			//등록헤더
		private SAPbouiCOM.DBDataSource oDS_PS_GA165H;
			//등록라인
		private SAPbouiCOM.DBDataSource oDS_PS_GA165L;
...
//*******************************************************************
		public void LoadForm(short prmSeq)
		{
			...
			oXmlDoc01.load(SubMain.ShareFolderPath + "ScreenPS\\PS_GA165.srf");
			...
			oFormUniqueID01 = "PS_GA165_" + GetTotalFormsCount();
			...
			oForm01.Freeze(true);
			PS_GA165_CreateItems();
			PS_GA165_ComboBox_Setting();
			PS_GA165_Initial_Setting(prmSeq);
			PS_GA165_CF_ChooseFromList();
			PS_GA165_EnableMenus();
			PS_GA165_SetDocument(Convert.ToString(prmSeq));
			PS_GA165_FormResize();
			...
			////BeforeAction = False
				...
				switch (pval.MenuUID) {
					...
					case "1282":
						...
						PS_GA165_FormItemEnabled();
						////UDO방식
						PS_GA165_AddMatrixRow(0, ref true);
						////UDO방식
						break;
					case "1288":
					case "1289":
					...
					case "1287":
						...
						oForm01.Freeze(true);
						oDS_PS_GA165H.SetValue("DocEntry", 0, "");

						for (i = 0; i <= oMat01.VisualRowCount - 1; i++) {
							oMat01.FlushToDataSource();
							oDS_PS_GA165H.SetValue("DocEntry", i, "");
		...
		private void Raise_EVENT_ITEM_PRESSED(ref object FormUID, ref SAPbouiCOM.ItemEvent pval, ref bool BubbleEvent)
		{
			...
			if (pval.BeforeAction == true) {
				...
				if (pval.ItemUID == "1") {
					if (oForm01.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE) {
						if (PS_GA165_DataValidCheck() == false) {
							BubbleEvent = false;
							return;
						}
				...
				if (pval.ItemChanged == true) {
					...
					if ((pval.ItemUID == "Mat01")) {
						...
						if (pval.ColUID == "DocDate") {
							...
							oDS_PS_GA165L.SetValue("U_" + pval.ColUID, pval.Row - 1, oMat01.Columns.Item(pval.ColUID).Cells.Item(pval.Row).Specific.VALUE);
							//                    Call oDS_PS_GA165L.setValue("U_CpName", pval.Row - 1, MDC_GetData.Get_ReData("U_CpName", "U_CpCode", "[@PS_PP001L]", "'" & oMat01.Columns(pval.ColUID).Cells(pval.Row).Specific.VALUE & "'")) '공정명
							oMat01.LoadFromDataSource();

							if (oMat01.RowCount == pval.Row & !string.IsNullOrEmpty(Strings.Trim(oDS_PS_GA165L.GetValue("U_" + pval.ColUID, pval.Row - 1)))) {
								PS_GA165_AddMatrixRow((pval.Row));
							}
							oMat01.LoadFromDataSource();
							...
							oDS_PS_GA165L.SetValue("U_ChkName", pval.Row - 1, MDC_GetData.Get_ReData("U_FULLNAME", "U_MSTCOD", "[OHEM]", "'" + oMat01.Columns.Item(pval.ColUID).Cells.Item(pval.Row).Specific.VALUE + "'"));
							//성명
							//                    Call oDS_PS_GA165L.setValue("U_TeamCode", pval.Row - 1, MDC_GetData.Get_ReData("U_TeamCode", "U_MSTCOD", "[OHEM]", "'" & oMat01.Columns(pval.ColUID).Cells(pval.Row).Specific.VALUE & "'")) '소속팀
							//                    Call oDS_PS_GA165L.setValue("U_RspCode", pval.Row - 1, MDC_GetData.Get_ReData("U_RspCode", "U_MSTCOD", "[OHEM]", "'" & oMat01.Columns(pval.ColUID).Cells(pval.Row).Specific.VALUE & "'")) '소속담당
							oMat01.LoadFromDataSource();
		...
		private void Raise_EVENT_MATRIX_LOAD(ref object FormUID, ref SAPbouiCOM.ItemEvent pval, ref bool BubbleEvent)
		{
			...
			if (pval.BeforeAction == true) {
				...
				PS_GA165_FormItemEnabled();
				PS_GA165_AddMatrixRow(oMat01.VisualRowCount);
				////UDO방식
				oMat01.AutoResizeColumns();
			}
			return;
		...
		private void Raise_EVENT_ROW_DELETE(ref string FormUID, ref SAPbouiCOM.IMenuEvent pval, ref bool BubbleEvent)
		{
			...
			if ((oLastColRow01 > 0)) {
				if (pval.BeforeAction == true) {
					...
					oMat01.FlushToDataSource();
					oDS_PS_GA165L.RemoveRecord(oDS_PS_GA165L.Size - 1);
					oMat01.LoadFromDataSource();
					if (oMat01.RowCount == 0) {
						PS_GA165_AddMatrixRow(0);
					} else {
						if (!string.IsNullOrEmpty(Strings.Trim(oDS_PS_GA165L.GetValue("U_DocDate", oMat01.RowCount - 1)))) {
							PS_GA165_AddMatrixRow(oMat01.RowCount);
						}
					}
				}
			}
		...
		private bool PS_GA165_CreateItems()
		{
			bool functionReturnValue = false;
			 // ERROR: Not supported in C#: OnErrorStatement


			oDS_PS_GA165H = oForm01.DataSources.DBDataSources("@PS_GA165H");
			oDS_PS_GA165L = oForm01.DataSources.DBDataSources("@PS_GA165L");
			oMat01 = oForm01.Items.Item("Mat01").Specific;

			oMat01.SelectionMode = SAPbouiCOM.BoMatrixSelect.ms_NotSupported;
			oMat01.AutoResizeColumns();
			return functionReturnValue;
			PS_GA165_CreateItems_Error:

			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_CreateItems_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
			return functionReturnValue;
		}

		public void PS_GA165_ComboBox_Setting()
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			//    Call MDC_PS_Common.Combo_ValidValues_Insert("PS_GA165", "Mat01", "UseYN", "Y", "Y")
			//    Call MDC_PS_Common.Combo_ValidValues_Insert("PS_GA165", "Mat01", "UseYN", "N", "N")
			//    Call MDC_PS_Common.Combo_ValidValues_SetValueColumn(oMat01.Columns("UseYN"), "PS_GA165", "Mat01", "UseYN")
			...
			sQry = sQry + "                 b.U_CodeNm";
			sQry = sQry + "  FROM       [@PS_GA050L] b";
			sQry = sQry + "  WHERE      b.Code = '19'";
			sQry = sQry + "                 AND U_UseYN = 'Y'";
			sQry = sQry + "  ORDER BY  b.U_Seq";

			oMat01.Columns.Item("SWCtgr").ValidValues.Add("", "");
			MDC_Com.MDC_GP_MatrixSetMatComboList(oMat01.Columns.Item("SWCtgr"), sQry);

			//매트릭스-SW상태
			sQry = "            SELECT      b.U_Code,";
			sQry = sQry + "                 b.U_CodeNm";
			sQry = sQry + "  FROM       [@PS_GA050L] b";
			sQry = sQry + "  WHERE      b.Code = '20'";
			sQry = sQry + "                 AND U_UseYN = 'Y'";
			sQry = sQry + "  ORDER BY  b.U_Seq";

			oMat01.Columns.Item("Status").ValidValues.Add("", "");
			MDC_Com.MDC_GP_MatrixSetMatComboList(oMat01.Columns.Item("Status"), sQry);
			...
			return;
			PS_GA165_ComboBox_Setting_Error:
			//UPGRADE_NOTE: oCombo 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
			oCombo = null;
			//UPGRADE_NOTE: oRecordSet01 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
			oRecordSet01 = null;
			oForm01.Freeze(false);
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_ComboBox_Setting_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public void PS_GA165_CF_ChooseFromList()
		{
			 // ERROR: Not supported in C#: OnErrorStatement

			return;
			PS_GA165_CF_ChooseFromList_Error:
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_CF_ChooseFromList_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public void PS_GA165_FormItemEnabled()
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			oForm01.Freeze(false);
			return;
			PS_GA165_FormItemEnabled_Error:
			oForm01.Freeze(false);
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_FormItemEnabled_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public void PS_GA165_AddMatrixRow(int oRow, ref bool RowIserted = false)
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			////행추가여부
			if (RowIserted == false) {
				oDS_PS_GA165L.InsertRecord((oRow));
			}
			oMat01.AddRow();
			oDS_PS_GA165L.Offset = oRow;
			oDS_PS_GA165L.SetValue("U_LineNum", oRow, Convert.ToString(oRow + 1));
			oMat01.LoadFromDataSource();
			oForm01.Freeze(false);
			return;
			PS_GA165_AddMatrixRow_Error:
			oForm01.Freeze(false);
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_AddMatrixRow_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public void PS_GA165_FormClear()
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			//UPGRADE_WARNING: MDC_GetData.Get_ReData() 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			DocEntry = MDC_GetData.Get_ReData(ref "AutoKey", ref "ObjectCode", ref "ONNM", ref "'PS_GA165'", ref "");
			...
			return;
			PS_GA165_FormClear_Error:
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_FormClear_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public void PS_GA165_Initial_Setting(short prmSeq)
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			return;
			PS_GA165_Initial_Setting_Error:
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_Initial_Setting_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		private void PS_GA165_EnableMenus()
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			////메뉴설정
			return;
			PS_GA165_EnableMenus_Error:
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_EnableMenus_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		private void PS_GA165_SetDocument(string prmSeq)
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			sQry = "SELECT COUNT(*) FROM [@PS_GA165H] WHERE U_Seq = " + prmSeq;
			...
			return;
			PS_GA165_SetDocument_Error:

			//UPGRADE_NOTE: oRecordSet01 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
			oRecordSet01 = null;
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_SetDocument_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}


		public bool PS_GA165_DataValidCheck()
		{
			bool functionReturnValue = false;
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			for (i = 1; i <= oMat01.VisualRowCount - 1; i++) {
				...
				//SW내역
				//UPGRADE_WARNING: oMat01.Columns(SWCts).Cells(i).Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				if ((string.IsNullOrEmpty(oMat01.Columns.Item("SWCts").Cells.Item(i).Specific.VALUE))) {
					SubMain.Sbo_Application.SetStatusBarMessage("SW내역은 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
					oMat01.Columns.Item("SWCts").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular);
					functionReturnValue = false;
					return functionReturnValue;
				}

				//SW분류
				//UPGRADE_WARNING: oMat01.Columns(SWCtgr).Cells(i).Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				if ((string.IsNullOrEmpty(oMat01.Columns.Item("SWCtgr").Cells.Item(i).Specific.VALUE))) {
					SubMain.Sbo_Application.SetStatusBarMessage("SW분류는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
					oMat01.Columns.Item("SWCtgr").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular);
					functionReturnValue = false;
					return functionReturnValue;
				}

				//SW상태
				//UPGRADE_WARNING: oMat01.Columns(Status).Cells(i).Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				if ((string.IsNullOrEmpty(oMat01.Columns.Item("Status").Cells.Item(i).Specific.VALUE))) {
					SubMain.Sbo_Application.SetStatusBarMessage("상태는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
					oMat01.Columns.Item("Status").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular);
					functionReturnValue = false;
					return functionReturnValue;
				}
			...
			return functionReturnValue;
			PS_GA165_DataValidCheck_Error:
			functionReturnValue = false;
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_DataValidCheck_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
			return functionReturnValue;
		}


		private void PS_GA165_MTX01()
		{
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			for (i = 0; i <= RecordSet01.RecordCount - 1; i++) {
				if (i != 0) {
					oDS_PS_GA165L.InsertRecord((i));
				}
				oDS_PS_GA165L.Offset = i;
				oDS_PS_GA165L.SetValue("U_COL01", i, RecordSet01.Fields.Item(0).Value);
				oDS_PS_GA165L.SetValue("U_COL02", i, RecordSet01.Fields.Item(1).Value);
			...
			RecordSet01 = null;
			oForm01.Freeze(false);
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_MTX01_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}


		private void PS_GA165_FormResize()
		{
			 // ERROR: Not supported in C#: OnErrorStatement


			return;
			PS_GA165_FormResize_Error:
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_FormResize_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
		}

		public bool PS_GA165_Validate(string ValidateType)
		{
			bool functionReturnValue = false;
			 // ERROR: Not supported in C#: OnErrorStatement
			...
			RecordSet01 = null;
			return functionReturnValue;
			PS_GA165_Validate_Exit:
			//UPGRADE_NOTE: RecordSet01 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
			RecordSet01 = null;
			return functionReturnValue;
			PS_GA165_Validate_Error:
			functionReturnValue = false;
			SubMain.Sbo_Application.SetStatusBarMessage("PS_GA165_Validate_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
			return functionReturnValue;
		}
	}
}
