		...
		private void PS_HR413_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			...
			try
			{
				...
				else if (oUID == "RateCode")
				{
					...
					sQry = sQry + "         and U_Year = '" + oForm.Items.Item("Year").Specific.Value.ToString().Trim() + "'";
					sQry = sQry + "         And b.U_RateCode = '" + oForm.Items.Item("RateCode").Specific.Value.ToString().Trim() + "'";
					oRecordSet.DoQuery(sQry);
