...
namespace PSH_BOne_AddOn
{
	...
	/// 전문직인사평가 결과조회(담당조회용)
	/// </summary>
	internal class PS_HR424 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFromDocEntry01)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_HR424.srf");
				...
				oFormUniqueID = "PS_HR424_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_HR424");
		...
		/// PS_HR424_CreateItems
		/// </summary>
		private void PS_HR424_CreateItems()
		{
			string sQry;
			...
			try
			{
				...
				oForm.DataSources.UserDataSources.Add("PassWd", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 20);
				oForm.Items.Item("PassWd").Specific.DataBind.SetBound(true, "", "PassWd");

                oForm.DataSources.UserDataSources.Item("MSTCOD").Value = dataHelpClass.User_MSTCOD();
                sQry = "SELECT U_FullName From [@PH_PY001A] Where Code = '" + dataHelpClass.User_MSTCOD() + "'";
                oRecordSet.DoQuery(sQry);
				oForm.DataSources.UserDataSources.Item("FullName").Value = oRecordSet.Fields.Item(0).Value.ToString().Trim();
		...
		/// PS_HR424_ComboBox_Setting
		/// </summary>
		private void PS_HR424_ComboBox_Setting()
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
			...
			try
			{
				...
				oForm.Items.Item("Group").Specific.ValidValues.Add("3", "임금피크");
		...
		/// <summary>
		/// PS_HR424_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_HR424_HeaderSpaceLineDel()
		{
		...
		/// </summary>
		/// <returns></returns>
		private bool PS_HR424_PasswordChk()
		{
			bool functionReturnValue = false;
			string errMessage = string.Empty;
			string MSTCOD;
		...
		/// <summary>
		/// PS_HR424_Print_Query
		/// </summary>
		[STAThread]
		private void PS_HR424_Print_Query()
		{
			string WinTitle;
			string ReportName;
			string Year;
        ...
        private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            try
            {
                if (pVal.BeforeAction == true)
                {
                    ...
                    {
                        if (PS_HR424_HeaderSpaceLineDel() == false)
                        {
                            BubbleEvent = false;
                            return;
                        }
                        else
                        {
                            ...
                            {
                                System.Threading.Thread thread = new System.Threading.Thread(PS_HR424_Print_Query);
