    ...
    internal class PS_FTP : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFromDocEntry01)
        {
            ...
            try
            {
                ...
                oForm.Freeze(true);
                PS_FTP_CreateItems();
                PS_FTP_ComboBox_Setting();
                PS_FTP_CF_ChooseFromList();
                PS_FTP_EnableMenus();
                PS_FTP_SetDocument(oFromDocEntry01);
                PS_FTP_FormResize();
        ...
        private void PS_FTP_Trans_FTP()
        {
            ...
            try
            {
                ...
                hOpen = 0;
                hConnection = 0;
                hOpen = modWinInet.InternetOpen(modWinInet.scUserAgent, modWinInet.INTERNET_OPEN_TYPE_DIRECT, Constants.vbNullString, Constants.vbNullString, 0);
                int nFlag = 0;
                nFlag = modWinInet.INTERNET_FLAG_PASSIVE;
                hConnection = modWinInet.InternetConnect(hOpen, FTP_SERVER, modWinInet.INTERNET_INVALID_PORT_NUMBER, FTP_USER, FTP_PASSWORD, modWinInet.INTERNET_SERVICE_FTP, nFlag, 0);

                Dir_Renamed = oForm.Items.Item("Dir").Specific.VALUE;

                oFileName = oForm.Items.Item("FileName").Specific.VALUE;

                pos = Strings.InStr(oFileName, ".");
                FileName = Convert.ToString(oForm.Items.Item("E_BANFN").Specific.VALUE + oForm.Items.Item("E_BNFPO").Specific.VALUE + Convert.ToDouble(Strings.Trim(Strings.Mid(oFileName, pos, 10))));

                //UpLoad Dir, "ftp/trans/group/" & FileName, ASCII
                UpLoad(ref Dir_Renamed, ref "trans/group/" + FileName, ref ASCII);

                //세션 종료처리
                if (hConnection != 0)
                    modWinInet.InternetCloseHandle(hConnection);
                if (hOpen != 0)
                    modWinInet.InternetCloseHandle(hOpen);
                hConnection = 0;
                hOpen = 0;
                ...
                MDC_Globals.oSapConnection01.Connection.SystemNumber = "01";
                //    oSapConnection01.Connection.SystemNumber = "00" '192.1.11.3 일때는 "00"
                if (!MDC_Globals.oSapConnection01.Connection.Logon(0, true))
                {
                    MDC_Com.MDC_GF_Message(ref "R/3 서버에 접속할수 없습니다.", ref "E");
                    goto FTP_Trans_Exit;
                }

                object oFunction01 = null;
                oFunction01 = MDC_Globals.oSapConnection01.Add("ZMM_INTF_GROUP_FILE");

                if (oFunction01 == null)
                {
                    MDC_Com.MDC_GF_Message(ref "함수(ZMM_INTF_GROUP_FILE) 생성오류.", ref "E");
                    goto FTP_Trans_Exit;
                }
                ...
                if (!oFunction01.Call)
                {
                    MDC_Com.MDC_GF_Message(ref "통합구매(R/3) 함수(ZMM_INTF_GROUP_FILE)호출중 오류발생", ref "E");
                    goto FTP_Trans_Exit;
                }
                else
                {
        ...
        /// </summary>
        /// <returns></returns>
        private bool PS_FTP_MatrixSpaceLineDel()
        {
            bool ReturnValue = false;
            int i;
            string errMessage = string.Empty;
        ...
        /// Delete_EmptyRow
        /// </summary>
        private void PS_FTP_Delete_EmptyRow()
        {
            int i;
            string errMessage = string.Empty;
        ...
        private void PS_FTP_FileSearch()
        {
            ...
            try
            {
                sFile = My.MyProject.Forms.FileListBoxForm.OpenDialog(ref FileListBoxForm, ref "*.*", ref "파일선택", ref "C:\\");
                if (string.IsNullOrEmpty(sFile))
                {
                    SubMain.Sbo_Application.StatusBar.SetText("파일을 선택해 주세요.", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                    return;
                }
                else
                {
                    ...
                    pos = Strings.InStr(sFile, "\\");

                    FileName = Strings.Trim(Strings.Mid(sFile, pos + 1, 100));

                    pos = Strings.InStr(FileName, "\\");
                    if (pos > 0)
                    {
                        FileName = Strings.Trim(Strings.Mid(FileName, pos + 1, 100));
                        pos = Strings.InStr(FileName, "\\");
                        if (pos > 0)
                        {
                            FileName = Strings.Trim(Strings.Mid(FileName, pos + 1, 100));
                            ...
                            if (pos > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    oForm.Items.Item("FileName").Specific.VALUE = FileName;
                                }
                            }
                            else
                            {
        ...
        /// <returns></returns>
        private bool PS_FTP_Validate(string ValidateType)
        {
            bool returnValue = false;
            int i;
            int j;
        ...
        /// FormResize
        /// </summary>
        private void PS_FTP_FormResize()
        {
            try
            {
            }
        ...
        /// 모드에 따른 아이템 설정
        /// </summary>
        private void PS_FTP_FormItemEnabled()
        {
            try
            {
            }
        ...
        private void PS_FTP_AddMatrixRow(int oRow, bool RowIserted)
        {
            try
            {
            }
        ...
        /// PS_FTP_etBaseForm
        /// </summary>
        private void PS_FTP_SetBaseForm()
        {
            string errMessage = string.Empty;
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        private bool PS_FTP_DataValidCheck()
        {
            bool ReturnValue = false;
            int i = 0;
            string errMessage = string.Empty;
        ...
        private void PS_FTP_FlushToItemValue(string oUID, int oRow, string oCol)
        {
            SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            ...
            try
            {
                if (pVal.Before_Action == true)
                {
                    if (pval.CharPressed == 9)
                    {
                        ...
                        //                        If oForm.Items("ItmBsort").Specific.VALUE = "" Then
