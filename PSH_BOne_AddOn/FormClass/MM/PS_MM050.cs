...
namespace PSH_BOne_AddOn
{
    ...
    internal class PS_MM050 : PSH_BaseClass
    {
        ...
        private SAPbouiCOM.DBDataSource oDS_PS_MM050H; //?±Î°ù?§Îçî
        private SAPbouiCOM.DBDataSource oDS_PS_MM050L; //?±Î°ù?ºÏù∏
        ...
        private void PS_MM050_ComboBox_Setting()
        {
            ...
            try
            {
                ...
                //?àÏùò?ÅÌÉú
                oForm.Items.Item("POStatus").Specific.ValidValues.Add("Y", "?πÏù∏");
        ...
        private void PS_MM050_Initialization()
        {
            ...
            try
            {
                ...
                oForm.Items.Item("CntcCode").Specific.Value = dataHelpClass.User_MSTCOD();//?ÑÏù¥?îÎ≥Ñ ?¨Î≤à ?∏ÌåÖ
                oForm.Items.Item("POStatus").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);
        ...
        private bool PS_MM050_MatrixSpaceLineDel()
        {
            ...
            try
            {
                ...
                {
                    errMessage = "?ºÏù∏ ?∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§. ?ïÏù∏?òÏÑ∏??.";
                    throw new Exception();
                }
                else
                {
                    for (i = 1; i <= oMat02.VisualRowCount; i++)
                    {
                        ...
                        {
                            if (PSH_Globals.SBO_Application.MessageBox(oRecordSet.Fields.Item(1).Value + "Í≥ÑÏÜç ÏßÑÌñâ?òÏãúÍ≤†Ïäµ?àÍπå?", 2, "Yes", "No") == 2)
                            {
                                errCode = 1;
                                throw new Exception();
                            }
                        ...
                        {
                            errMessage = Convert.ToString(ErrRowCount) + "?âÏùò ?òÎüâ?? Í∞íÏù¥ ?ÜÏäµ?àÎã§. ?ïÏù∏Î∞îÎûç?àÎã§.";
                            ErrRowCount = i;
                            throw new Exception();
                        }
                        else if (PS_MM050_CheckDate(oMat02.Columns.Item("PODocNum").Cells.Item(i).Specific.Value) == false)
                        {
                            errMessage = ErrRowCount + "?? [" + oMat01.Columns.Item("ItemCode").Cells.Item(ErrRowCount + 1).Specific.Value + "]?? Í∞Ä?ÖÍ≥†?ºÏ? Íµ¨Îß§?àÏùò?ºÍ≥º Í∞ôÍ±∞?? ??ñ¥?ºÌï©?àÎã§. ?ïÏù∏?òÏã≠?úÏò§. ?¥Îãπ Í∞Ä?ÖÍ≥†?? ?ÑÏ≤¥Í∞Ä ?±Î°ù?òÏ? ?äÏäµ?àÎã§.";
                            ErrRowCount = i;
                            throw new Exception();
                        }
                    }
            ...
            catch (Exception ex)
            {
                if (errCode == 1)
                {
                    PSH_Globals.SBO_Application.MessageBox("Ï∑®ÏÜåÏ≤òÎ¶¨?òÏóà?µÎãà??.");
                }
        ...
        private bool PS_MM050_Display_MatrixData()
        {
            ...
            try
            {
                ...
                {
                    errMessage = "Ï°∞Ìöå Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§. ?ïÏù∏?òÏÑ∏??.";
                    throw new Exception();
                }
                ...
                if (oRecordSet.RecordCount > 0)
                {
                    while (!oRecordSet.EoF)
                    {
                        ...
                        oDS_PS_TEMPTABLE.SetValue("U_sField10", sCnt, oRecordSet.Fields.Item("U_ReqCntc").Value.ToString().Trim()); //Ï≤?µ¨??
                        oDS_PS_TEMPTABLE.SetValue("U_sField07", sCnt, oRecordSet.Fields.Item("U_OrdNum").Value.ToString().Trim()); //?ëÎ≤à
                        oDS_PS_TEMPTABLE.SetValue("U_sField08", sCnt, oRecordSet.Fields.Item("U_OrdSub1").Value.ToString().Trim()); //Sub?ëÎ≤à1
                        oDS_PS_TEMPTABLE.SetValue("U_sField09", sCnt, oRecordSet.Fields.Item("U_OrdSub2").Value.ToString().Trim()); //Sub?ëÎ≤à2
                        oDS_PS_TEMPTABLE.SetValue("U_sField17", sCnt, oRecordSet.Fields.Item("U_DocCur").Value.ToString().Trim()); //?µÌôî
                        oDS_PS_TEMPTABLE.SetValue("U_qField04", sCnt, oRecordSet.Fields.Item("U_DocRate").Value.ToString().Trim()); //?òÏú®
                        oDS_PS_TEMPTABLE.SetValue("U_qField03", sCnt, oRecordSet.Fields.Item("U_FCPrice").Value.ToString().Trim()); //?∏Ìôî?®Í?
                        oDS_PS_TEMPTABLE.SetValue("U_aField03", sCnt, oRecordSet.Fields.Item("U_FCAmount").Value.ToString().Trim()); //?∏ÌôîÍ∏àÏï°
                        sCnt += 1;
                        oRecordSet.MoveNext();
                    }
                }
            ...
            finally
            {
                oForm.Freeze(false);
                System.Runtime.InteropServices.Marshal.ReleaseComObject(oRecordSet);
                System.Runtime.InteropServices.Marshal.ReleaseComObject(ProgressBar01); //Î©îÎ™®Î¶? ?¥Ï†ú
            }
            return returnValue;
        }

        /// <summary>
        /// ?†Ìñâ?ÑÎ°ú?∏Ïä§?Ä ?ºÏûê ÎπÑÍµê
        /// </summary>
        /// <returns>true:?†Ìñâ?ÑÎ°ú?∏Ïä§Î≥¥Îã§ ?ºÏûêÍ∞Ä Í∞ôÍ±∞?? ?êÎ¶¥ Í≤ΩÏö∞, false:?†Ìñâ?ÑÎ°ú?∏Ïä§Î≥¥Îã§ ?ºÏûêÍ∞Ä Îπ†Î? Í≤ΩÏö∞</returns>
            ...
            try
            {
                ...
                else if (ValidateType == "?âÏÇ≠??")
                {
                    if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                errMessage = "?†ÌÉù?? ?àÎ™©?Ä ?¥Î? Í≤Ä?òÏûÖÍ≥? ?òÏóà?µÎãà??(Í≤Ä?òÏûÖÍ≥? Î¨∏ÏÑúÎ≤àÌò∏ : " + oRecordSet.Fields.Item("DocEntry").Value + "-" + oRecordSet.Fields.Item("LineNum").Value + "). ?¥Îãπ Í∞Ä?ÖÍ≥†Î¨∏ÏÑúÎ•? ?âÏÇ≠?úÌï† ?? ?ÜÏäµ?àÎã§.";
                                throw new Exception();
                            }
                        }
                    }
                ...
                else if (ValidateType == "Ï∑®ÏÜå")
                {
                    for (i = 1; i < oMat02.VisualRowCount; i++)
                    {
                        ...
                        if (oRecordSet.Fields.Item("Count").Value > 0)
                        {
                            errMessage = i + "?? ?àÎ™©?? ?¥Î? Í≤Ä?òÏûÖÍ≥? ?òÏóà?µÎãà??(Í≤Ä?òÏûÖÍ≥? Î¨∏ÏÑúÎ≤àÌò∏ : " + oRecordSet.Fields.Item("DocEntry").Value + "-" + oRecordSet.Fields.Item("LineNum").Value + "). ?¥Îãπ Í∞Ä?ÖÍ≥†Î¨∏ÏÑúÎ•? Ï∑®ÏÜå?? ?? ?ÜÏäµ?àÎã§.";
                            throw new Exception();
                        }
                    }
                }
                else if (ValidateType == "?´Í∏∞")
                {
                    for (i = 1; i < oMat02.VisualRowCount; i++)
                    {
                        sQry = " SELECT    COUNT(*) AS [Count],";
                        ...
                        if (oRecordSet.Fields.Item("Count").Value > 0)
                        {
                            errMessage = i + "?? ?àÎ™©?? ?¥Î? Í≤Ä?òÏûÖÍ≥? ?òÏóà?µÎãà??(Í≤Ä?òÏûÖÍ≥? Î¨∏ÏÑúÎ≤àÌò∏ : " + oRecordSet.Fields.Item("DocEntry").Value + "-" + oRecordSet.Fields.Item("LineNum").Value + "). ?¥Îãπ Í∞Ä?ÖÍ≥†Î¨∏ÏÑúÎ•? ?´Í∏∞?? ?? ?ÜÏäµ?àÎã§.";
                            throw new Exception();
                        }
                    }
                }
        ...
        private void PS_MM050_Delete_EmptyRow()
        {
            ...
            try
            {
                ...
                for (i = 0; i < oMat02.VisualRowCount; i++)
                {
                    ...
                    {
                        oDS_PS_MM050L.RemoveRecord(i); // Mat01?? ÎßàÏ?ÎßâÎùº??(ÎπàÎùº??) ??†ú
                    }
                }
        ...
        /// </summary>
        /// <param name="oRow">?? Î≤àÌò∏</param>
        /// <param name="RowIserted">?? Ï∂îÍ? ?¨Î?</param>
        ...
        /// <summary>
        /// FlushToItemValue(?¨Ïö©?êÏùò Event?? ?∞Î•∏ ?îÎ©¥ Item?? ?†Îèô?ÅÏù∏ ?∏ÌåÖ)
        /// </summary>
            ...
            try
            {
                ...
                switch (oUID)
                {
                    ...
                    case "Mat01":
                        ...
                        sQry = "Select a.DocNum, b.LineId, b.U_ItemCode "; //U_LineNumÎ•? LineIdÎ°? ?òÏ†ï(2012.07.24 ?°Î™ÖÍ∑?)
                    ...
                    case "Mat02":
                        if (oCol == "Weight")
                        {
                            ...
                            {
                                //Í∏àÏï° Î∞òÏò¨Î¶? 2012/04/03 ?∏Í∑º?? ?òÏ†ï
            ...
            try
            {
                ...
                if (pVal.BeforeAction == true)
                {
                    if (pVal.ItemUID == "1")
                    {
                        ...
                        PS_MM050_Delete_EmptyRow(); //Í≤Ä?òÏûÖÍ≥? Î¨∏ÏÑúÎß? ?±Î°ù ?? ?? ?âÏ? Ï£ºÏÑù ?úÏô∏
                ...
                else if (pVal.BeforeAction == false)
                {
                    if (pVal.ItemUID == "1")
                    {
                        ...
                        else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE && pVal.Action_Success == true) // ?©ÏòÅ?? 2018.07.20 Call Sbo_Application.ActivateMenuItem("1282") ?§Î•òÎ°? Menu Event ?¥Ïóê ?àÎäî Í≤ÉÏùÑ Í∞Ä?∏Ïò¥.
                        {
                            PS_MM050_Initialization();
                            PS_MM050_FormItemEnabled();
                            PS_MM050_FormClear();
                        ...
                        else if (pVal.ItemUID == "Mat02")
                        {
                            if (pVal.ColUID == "Qty")
                            {
                                ...
                                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                                {
                                    ...
                                    {
                                        //Í∏∞Í≥Ñ?¨ÏóÖÎ∂Ä ?∏Ï£ºÍ∞ÄÍ≥?, ?∏Ï£º?úÏûë Ï§ëÎüâ?Ä ?òÎüâÍ≥? ?ôÏùº?òÎã§.
            ...
            try
            {
                ...
                if (pVal.Before_Action == true)
                {
                    if (pVal.ItemUID == "Mat01" && pVal.Row != 0)
                    {
                        ...
                        //?àÏùòÍ∏àÏï° VS Í∞Ä?ÖÍ≥† Í∏àÏï° ÎπÑÍµê(?ÅÎã® Î©îÌä∏Î¶?ä§?? Í∞íÏùÑ Ï°∞Ìöå ?¥ÎÜì?Ä ?ÅÌÉú?êÏÑú ?àÏùòÍ∏àÏï°?? Î≥ÄÍ≤ΩÌïú Í≤ΩÏö∞ Ï≤¥ÌÅ¨)
                        for (i = 0; i < oMat02.VisualRowCount; i++)
                        {
                            ...
                            {
                                errMessage = "Í∞ôÏ? ?âÏùÑ ?êÎ≤à ?†ÌÉù?? ?? ?ÜÏäµ?àÎã§. ?ïÏù∏?òÏÑ∏??.";
                                throw new Exception();
                                //j = 1;
                            }
                        }
                        ...
                        //?ÖÍ≥†?òÎüâÍ≥? ÎØ∏ÏûÖÍ≥? ?òÎüâ?? Í∞ôÏùÑ?? ÎØ∏ÏûÖÍ≥? ?òÎüâ?? ?ÖÍ≥†?òÎüâ?? ?£Ïñ¥Ï§Ä??
                        ...
                        oMat02.Columns.Item("DocCur").Cells.Item(oMat02.VisualRowCount).Specific.Value = oDS_PS_TEMPTABLE.GetValue("U_sField17", pVal.Row - 1).ToString().Trim(); //?µÌôî
                        oMat02.Columns.Item("DocRate").Cells.Item(oMat02.VisualRowCount).Specific.Value = oDS_PS_TEMPTABLE.GetValue("U_qField04", pVal.Row - 1).ToString().Trim(); //?òÏú®
                        oMat02.Columns.Item("FCPrice").Cells.Item(oMat02.VisualRowCount).Specific.Value = oDS_PS_TEMPTABLE.GetValue("U_qField03", pVal.Row - 1).ToString().Trim(); //?∏Ìôî?òÏÇ∞?®Í?
                        oMat02.Columns.Item("FCAmount").Cells.Item(oMat02.VisualRowCount).Specific.Value = oDS_PS_TEMPTABLE.GetValue("U_aField03", pVal.Row - 1).ToString().Trim(); //?∏Ìôî?òÏÇ∞Í∏àÏï°
                        ...
                        //Í∞ÄÍ≥µÎπÑ?àÏùò(30), ?∏Ï£º?úÏûë?àÏùò(40) (2011.10.28 ?°Î™ÖÍ∑? ?òÏ†ï)
                        ...
                        {
                            oMat02.Columns.Item("QEYesNo").Cells.Item(oMat02.VisualRowCount).Specific.Select("Y"); //Í∞ÄÍ≥µÎπÑ?àÏùò(30), ?∏Ï£º?úÏûë?àÏùò(40) ??
                        }
                        else
                        {
                    ...
                    else if (pVal.ItemUID == "Mat01" && pVal.Row == 0) //?îÎ∏î?¥Î¶≠ ?? ?ïÎ†¨
                    {
                    ...
                    else if (pVal.ItemUID == "Mat02")
                    {
                        if (pVal.ColUID == "QEYesNo")
                        {
                            ...
                            {
                                selectedValue = oMat02.Columns.Item("QEYesNo").Cells.Item(1).Specific.Selected.Value; //Ï≤? ?âÏóê ?†ÌÉù?? Í∞íÏùÑ ?Ä??
                ...
                {
                    if (pVal.ItemUID == "Mat01" && pVal.Row == 0) //?îÎ∏î?¥Î¶≠ ?? ?ïÎ†¨
                    {
                        oMat01.FlushToDataSource();
        ...
        private void Raise_EVENT_ROW_DELETE(string FormUID, ref SAPbouiCOM.MenuEvent pVal, ref bool BubbleEvent)
        {
            ...
            try
            {
                ...
                if (oLastColRow01 > 0)
                {
                    if (pVal.BeforeAction == true)
                    {
                        if (PS_MM050_Validate("?âÏÇ≠??") == false)
                        {
                            BubbleEvent = false;
                            return;
                        }
            ...
            try
            {
                ...
                if (pVal.BeforeAction == true)
                {
                    switch (pVal.MenuUID)
                    {
                        case "1284": //Ï∑®ÏÜå
                            if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                            {
                                if (PS_MM050_Validate("Ï∑®ÏÜå") == false)
                                {
                                    BubbleEvent = false;
                                    return;
                                }
                                if (PSH_Globals.SBO_Application.MessageBox("?ïÎßêÎ°? Ï∑®ÏÜå?òÏãúÍ≤†Ïäµ?àÍπå?", Convert.ToInt32("1"), "??", "?ÑÎãà??") != Convert.ToDouble("1"))
                                {
                                    BubbleEvent = false;
                                    return;
                                }
                            ...
                            {
                                dataHelpClass.MDC_GF_Message("?ÑÏû¨ Î™®Îìú?êÏÑú?? Ï∑®ÏÜå?†Ïàò ?ÜÏäµ?àÎã§.", "W");
                                BubbleEvent = false;
                                return;
                            }
                            break;
                        case "1286": //?´Í∏∞
                            if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                            {
                                if (PS_MM050_Validate("?´Í∏∞") == false)
                                {
                                    BubbleEvent = false;
                                    return;
                                }
                                if (PSH_Globals.SBO_Application.MessageBox("?ïÎßêÎ°? ?´Í∏∞?òÏãúÍ≤†Ïäµ?àÍπå?", Convert.ToInt32("1"), "??", "?ÑÎãà??") != Convert.ToDouble("1"))
                                {
                                    BubbleEvent = false;
                                    return;
                                }
                            ...
                            {
                                dataHelpClass.MDC_GF_Message("?ÑÏû¨ Î™®Îìú?êÏÑú?? ?´Í∏∞?†Ïàò ?ÜÏäµ?àÎã§.", "W");
                                BubbleEvent = false;
                                return;
                            }
                            break;
                        ...
                        case "1282": //Ï∂îÍ?
                            break;
                        case "1288": //?àÏΩî?úÏù¥??(ÏµúÏ¥à)
                        case "1289": //?àÏΩî?úÏù¥??(?¥Ï†Ñ)
                        case "1290": //?àÏΩî?úÏù¥??(?§Ïùå)
                        case "1291": //?àÏΩî?úÏù¥??(ÏµúÏ¢Ö)
                            break;
                    }
                }
                else if (pVal.BeforeAction == false)
                {
                    switch (pVal.MenuUID)
                    {
                        ...
                        case "1281": //Ï∞æÍ∏∞
                            PS_MM050_FormItemEnabled();
                            oForm.Items.Item("BPLId").Specific.Select(dataHelpClass.User_BPLID(), SAPbouiCOM.BoSearchKey.psk_ByValue); //?ÑÏù¥?îÎ≥Ñ ?¨ÏóÖ?? ?∏ÌåÖ
                            if (dataHelpClass.User_SuperUserYN() == "N") //?òÌçº?†Ï??? Í≤ΩÏö∞?? ?¨Î≤à ÎØ∏ÌëúÍ∏?
                            {
