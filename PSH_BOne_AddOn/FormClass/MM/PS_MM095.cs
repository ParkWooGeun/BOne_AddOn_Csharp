            ...
            try
            {
                if (pVal.BeforeAction == true)
                {
                    switch (pVal.MenuUID)
                    {
                        case "1284": //취소
                            // 마감일자 Check
                            if (dataHelpClass.Check_Finish_Status(oDS_PS_MM095H.GetValue("U_BPLId", 0).ToString().Trim(), oDS_PS_MM095H.GetValue("U_DocDate", 0).ToString().Trim().Substring(0, 6)) == false)
                            {
