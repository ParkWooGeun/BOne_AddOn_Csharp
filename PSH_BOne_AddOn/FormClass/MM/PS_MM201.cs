...
namespace PSH_BOne_AddOn
{
	...
	/// 품목수불명세서(연간)
	/// </summary>
	internal class PS_MM201 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM201.srf");
				...
				oFormUniqueID = "PS_MM201_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM201");
				...
				oForm.Freeze(true);
				PS_MM201_CreateItems();
				PS_MM201_ComboBox_Setting();
		...
		/// PS_MM201_CreateItems
		/// </summary>
		private void PS_MM201_CreateItems()
		{
			try
			{
				...
				oForm.DataSources.UserDataSources.Item("YYYY").Value = DateTime.Now.ToString("yyyy");
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// PS_MM201_ComboBox_Setting
		/// </summary>
		private void PS_MM201_ComboBox_Setting()
		{
			string sQry;
		...
		/// <summary>
		/// PS_MM201_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_MM201_HeaderSpaceLineDel()
		{
			...
			try
			{
				if (string.IsNullOrEmpty(oForm.Items.Item("YYYY").Specific.Value.ToString().Trim()))
				{
					errMessage = "년도는 필수사항입니다. 확인하여 주십시오.";
					throw new Exception();
				}
				ReturnValue = true;
			}
		...
		/// <summary>
		/// PS_MM201_Print_Query
		/// </summary>
		[STAThread]
		private void PS_MM201_Print_Query()
		{
			string WinTitle;
			string ReportName;
			string Gubun;
			...
			try
			{
				...
				WinTitle = "[PS_MM201] 제품수불명세서(연간)";
				ReportName = "PS_MM201_01.RPT";
				...
				if (Gubun == "10")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@Gubun", "제품"));
				}
				else if (Gubun == "20")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@Gubun", "제품(임가공)"));
				}
				else if (Gubun == "30")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@Gubun", "상품"));
				}
				else if (Gubun == "40")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@Gubun", "원재료"));
				}
				else
				{
					dataPackFormula.Add(new PSH_DataPackClass("@Gubun", ""));
				}

				//Parameter
				dataPackParameter.Add(new PSH_DataPackClass("@BPLId", BPLId));
				dataPackParameter.Add(new PSH_DataPackClass("@YEAR", YYYY));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM201_HeaderSpaceLineDel() == false)
						{
							BubbleEvent = false;
							return;
						}
						...
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_MM201_Print_Query);
