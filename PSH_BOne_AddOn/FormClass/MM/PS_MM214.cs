...
namespace PSH_BOne_AddOn
{
	/// <summary>
	/// 수불명세서
	/// 화면을 크게 키우면 항목이 나옴.. 전체적으로 이해안됨.. 
	/// </summary>
	internal class PS_MM214 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM214.srf");
				...
				oFormUniqueID = "PS_MM214_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM214");
				...
				oForm.Freeze(true);
				PS_MM214_CreateItems();
				PS_MM214_ComboBox_Setting();
		...
		/// PS_MM214_CreateItems
		/// </summary>
		private void PS_MM214_CreateItems()
		{
			try
			{
				...
				oForm.DataSources.UserDataSources.Item("Check02").Value = "N";
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// PS_MM214_ComboBox_Setting
		/// </summary>
		private void PS_MM214_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				oForm.Items.Item("AcctCode").Specific.ValidValues.Add("11502200", "임가공");
				oForm.Items.Item("AcctCode").Specific.ValidValues.Add("11501100", "상품");
				oForm.Items.Item("AcctCode").Specific.ValidValues.Add("11503100", "재공");
				oForm.Items.Item("AcctCode").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);

				// 창고
				oForm.Items.Item("WhsCode").Specific.ValidValues.Add("000", "제품");
				oForm.Items.Item("WhsCode").Specific.ValidValues.Add("901", "임가공");
				oForm.Items.Item("WhsCode").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);

				// 사업장
				oForm.Items.Item("BPLId").Specific.ValidValues.Add("1", "(주)풍산홀딩스 창원사업장");
				oForm.Items.Item("BPLId").Specific.ValidValues.Add("2", "(주)풍산홀딩스 부산사업장");
				oForm.Items.Item("BPLId").Specific.ValidValues.Add("6", "(주)풍산홀딩스 안강사업장");
		...
		/// <summary>
		/// PS_MM214_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_MM214_HeaderSpaceLineDel()
		{
		...
		/// <summary>
		/// PS_MM214_Print_Query
		/// </summary>
		[STAThread]
		private void PS_MM214_Print_Query()
		{
			...
			try
			{
				...
				if (Gubun == "1")
				{
					if (AcctCode == "11502200" && BPLId == "1") //창원/임가공 리포트 분기
					{
						WinTitle = "[PS_MM214_02] 수불명세서";
						ReportName = "PS_MM214_02.RPT";
					}
					else
					{
						WinTitle = "[PS_MM214_01] 수불명세서";
						ReportName = "PS_MM214_01.RPT";
					}
				}
				else if (Gubun == "2") // 레포트파일이 없슴 ..  사용 안하는것 같음
                {
					WinTitle = "[PS_MM214_04] 수불명세서(집계)";
					ReportName = "PS_MM214_04.RPT";
				}
				...
				if (BPLId == "6")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@BPLId", "(주)풍산홀딩스 안강사업장"));
				}
				else
				{
				...
				if (WhsCode == "000")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@WhsName", "전체"));
				}
				else
				{
					sQry = "SELECT WhsName From OWHS where WhsCode = '" + WhsCode + "'";
					oRecordSet.DoQuery(sQry);
					WhsName = oRecordSet.Fields.Item(0).Value.ToString().Trim();
					dataPackFormula.Add(new PSH_DataPackClass("@WhsName", WhsName));
				}
				...
				dataPackParameter.Add(new PSH_DataPackClass("@AcctCode", AcctCode));
				dataPackParameter.Add(new PSH_DataPackClass("@WareHouse", WhsCode));
				dataPackParameter.Add(new PSH_DataPackClass("@Wgt", ChkBox));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM214_HeaderSpaceLineDel() == false)
						{
							BubbleEvent = false;
							return;
						}
						...
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_MM214_Print_Query);
