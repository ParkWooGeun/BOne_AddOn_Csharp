...
namespace PSH_BOne_AddOn
{
	...
	/// 미품의현황
	/// </summary>
	internal class PS_MM215 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM215.srf");
				...
				oFormUniqueID = "PS_MM215_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM215");
				...
				oForm.Freeze(true);
				PS_MM215_CreateItems();
				PS_MM215_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM215_CreateItems
		/// </summary>
		private void PS_MM215_CreateItems()
		{
			try
			{
				...
				oForm.DataSources.UserDataSources.Add("POType", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 100);
				...
				//조회구분(미견적/미품의)
				oForm.DataSources.UserDataSources.Add("SrchCls", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 20);
				oForm.Items.Item("SrchCls").Specific.DataBind.SetBound(true, "", "SrchCls");
		...
		/// PS_MM215_ComboBox_Setting
		/// </summary>
		private void PS_MM215_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				// 품의형태
				oForm.Items.Item("PQType").Specific.ValidValues.Add("0", "전체조회");
				sQry = "SELECT Code, Name From [@PSH_RETYPE]";
				oRecordSet.DoQuery(sQry);
				while (!(oRecordSet.EoF))
				{
					oForm.Items.Item("PQType").Specific.ValidValues.Add(oRecordSet.Fields.Item(0).Value.ToString().Trim(), oRecordSet.Fields.Item(1).Value.ToString().Trim());
					oRecordSet.MoveNext();
				}
				oForm.Items.Item("PQType").Specific.Select("0", SAPbouiCOM.BoSearchKey.psk_Index);

				// 품의구분
				oForm.Items.Item("POType").Specific.ValidValues.Add("%", "전체");
				dataHelpClass.Set_ComboList(oForm.Items.Item("POType").Specific, "SELECT Code, Name From [@PSH_ORDTYP] Order by Code", "", false, false);
				oForm.Items.Item("POType").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);

				// 조회구분(미견적/미품의)
				oForm.Items.Item("SrchCls").Specific.ValidValues.Add("1", "미견적");
				oForm.Items.Item("SrchCls").Specific.ValidValues.Add("2", "미품의");
				oForm.Items.Item("SrchCls").Specific.Select(1, SAPbouiCOM.BoSearchKey.psk_Index);
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// <summary>
		/// PS_MM215_DataValidCheck
		/// </summary>
		/// <returns></returns>
		private bool PS_MM215_DataValidCheck()
		{
		...
		/// <summary>
		/// PS_MM215_Print_Report01 리포트 출력
		/// </summary>
		[STAThread]
		private void PS_MM215_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string BPLId;
			...
			try
			{
				...
				PQType = oForm.Items.Item("PQType").Specific.Value.ToString().Trim();
				DocDateFr = oForm.Items.Item("DocDateFr").Specific.Value.ToString().Trim();
				DocDateTo = oForm.Items.Item("DocDateTo").Specific.Value.ToString().Trim();
				ItemCode = oForm.Items.Item("ItemCode").Specific.Value.ToString().Trim();
				POType = oForm.Items.Item("POType").Specific.Value.ToString().Trim();
				SrchCls = oForm.Items.Item("SrchCls").Specific.Value.ToString().Trim();
				...
				dataPackParameter.Add(new PSH_DataPackClass("@POType", POType));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM215_DataValidCheck() == false)
						{
							BubbleEvent = false;
							return;
						}

						System.Threading.Thread thread = new System.Threading.Thread(PS_MM215_Print_Report01);
