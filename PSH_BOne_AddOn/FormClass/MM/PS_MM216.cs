...
namespace PSH_BOne_AddOn
{
	...
	/// 미입고현황
	/// </summary>
	internal class PS_MM216 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM216.srf");
				...
				oFormUniqueID = "PS_MM216_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM216");
				...
				oForm.Freeze(true);
				PS_MM216_CreateItems();
				PS_MM216_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM216_CreateItems
		/// </summary>
		private void PS_MM216_CreateItems()
		{
			try
			{
				oForm.DataSources.UserDataSources.Add("BPLId", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 100);
				oForm.DataSources.UserDataSources.Add("DueDateFr", SAPbouiCOM.BoDataType.dt_DATE, 100);
				oForm.DataSources.UserDataSources.Add("DueDateTo", SAPbouiCOM.BoDataType.dt_DATE, 100);
				...
				oForm.Items.Item("DueDateFr").Specific.Value = DateTime.Now.ToString("yyyyMM") + "01";
				oForm.Items.Item("DueDateTo").Specific.Value = DateTime.Now.ToString("yyyyMMdd");
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// PS_MM216_ComboBox_Setting
		/// </summary>
		private void PS_MM216_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				// 품목구분
				oForm.Items.Item("OrdType").Specific.ValidValues.Add("0", "전체품목");
				sQry = "SELECT Code, Name From [@PSH_ORDTYP] Order by Code";
				oRecordSet.DoQuery(sQry);
				while (!(oRecordSet.EoF))
				{
					oForm.Items.Item("OrdType").Specific.ValidValues.Add(oRecordSet.Fields.Item(0).Value.ToString().Trim(), oRecordSet.Fields.Item(1).Value.ToString().Trim());
					oRecordSet.MoveNext();
				}
				oForm.Items.Item("OrdType").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);

				//순환품여부
				oForm.Items.Item("RotateYN").Specific.ValidValues.Add("%", "전체");
				oForm.Items.Item("RotateYN").Specific.ValidValues.Add("N", "[N]일반품");
				oForm.Items.Item("RotateYN").Specific.ValidValues.Add("Y", "[Y]순환품");
				oForm.Items.Item("RotateYN").Specific.Select("%", SAPbouiCOM.BoSearchKey.psk_ByValue);
		...
		/// <summary>
		/// PS_MM216_DataValidCheck
		/// </summary>
		/// <returns></returns>
		private bool PS_MM216_DataValidCheck()
		{
			...
			try
			{
				if (string.IsNullOrEmpty(oForm.Items.Item("DueDateFr").Specific.Value.ToString().Trim()))
				{
					oForm.Items.Item("DueDateFr").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
					errMessage = "납기일자 From은 필수입니다.";
					throw new Exception();
				}
				if (string.IsNullOrEmpty(oForm.Items.Item("DueDateTo").Specific.Value.ToString().Trim()))
				{
					oForm.Items.Item("DueDateTo").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
					errMessage = "납기일자 To는 필수입니다.";
					throw new Exception();
				}
		...
		/// <summary>
		/// PS_MM216_Print_Report01 리포트 출력
		/// </summary>
		[STAThread]
		private void PS_MM216_Print_Report01()
		{
			string WinTitle;
			...
			try
			{
				...
				RotateYN = oForm.Items.Item("RotateYN").Specific.Value.ToString().Trim();
				ItemType = oForm.Items.Item("ItemType").Specific.Value.ToString().Trim();
				OptBtnValue = oForm.DataSources.UserDataSources.Item("OptionDS").Value.ToString().Trim();
				...
				else if (OptBtnValue == "B")
				{
					if (BPLId == "2")  //부산사업장일 경우는 별도 양식
					{
				...
				dataPackParameter.Add(new PSH_DataPackClass("@RotateYN", RotateYN));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM216_DataValidCheck() == false)
						{
							BubbleEvent = false;
							return;
						}

						System.Threading.Thread thread = new System.Threading.Thread(PS_MM216_Print_Report01);
