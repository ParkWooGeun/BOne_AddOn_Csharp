...
namespace PSH_BOne_AddOn
{
	...
	/// 거래처별 품의 대장
	/// </summary>
	internal class PS_MM224 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM224.srf");
				...
				oFormUniqueID = "PS_MM224_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM224");
				...
				oForm.Freeze(true);
				PS_MM224_CreateItems();
				PS_MM224_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM224_CreateItems
		/// </summary>
		private void PS_MM224_CreateItems()
		{
			try
			{
		...
		/// PS_MM224_ComboBox_Setting
		/// </summary>
		private void PS_MM224_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				//출력구분
				oForm.Items.Item("CanType").Specific.ValidValues.Add("1", "정상");
				oForm.Items.Item("CanType").Specific.ValidValues.Add("2", "취소&종료");
				oForm.Items.Item("CanType").Specific.Select("1", SAPbouiCOM.BoSearchKey.psk_ByValue);
		...
		/// <summary>
		/// PS_MM224_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		private void PS_MM224_FlushToItemValue(string oUID)
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
		...
		/// <summary>
		/// PS_MM224_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_MM224_HeaderSpaceLineDel()
		{
			...
			try
			{
				...
				{
					errMessage = "품의일자는 필수입력사항입니다. 확인하세요.";
					throw new Exception();
				}
		...
		/// <summary>
		/// PS_MM224_Print_Report01 리포트 출력
		/// </summary>
		[STAThread]
		private void PS_MM224_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string BPLId;
			...
			try
			{
				...
				PrtType = oForm.Items.Item("CanType").Specific.Value.ToString().Trim();
				...
				WinTitle = "거래처별 품의 대장 [PS_MM224_01]";
				ReportName = "PS_MM224_01.rpt";
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM224_HeaderSpaceLineDel() == false)
						{
							BubbleEvent = false;
							return;
						}

						System.Threading.Thread thread = new System.Threading.Thread(PS_MM224_Print_Report01);
			...
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "CardCode")
						{
							PS_MM224_FlushToItemValue(pVal.ItemUID);
						}
					}
				}
			}
