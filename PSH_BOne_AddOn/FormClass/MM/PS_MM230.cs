...
namespace PSH_BOne_AddOn
{
	...
	/// 업체전달사항등록
	/// </summary>
	internal class PS_MM230 : PSH_BaseClass
	{
		private string oFormUniqueID;
		private SAPbouiCOM.Matrix oMat;
			
		private SAPbouiCOM.DBDataSource oDS_PS_MM230H; //등록헤더
		private SAPbouiCOM.DBDataSource oDS_PS_MM230L; //등록라인
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM230.srf");
				...
				oFormUniqueID = "PS_MM230_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM230");
				...
				oForm.Freeze(true);
				PS_MM230_CreateItems();
				PS_MM230_ComboBox_Setting();
		...
		/// PS_MM230_CreateItems
		/// </summary>
		private void PS_MM230_CreateItems()
		{
			try
			{
				oDS_PS_MM230H = oForm.DataSources.DBDataSources.Item("@PS_MM230H");
				oDS_PS_MM230L = oForm.DataSources.DBDataSources.Item("@PS_MM230L");
		...
		/// PS_MM230_ComboBox_Setting
		/// </summary>
		private void PS_MM230_ComboBox_Setting()
		{
			...
			try
			{
				...
				//발송순번
				oForm.Items.Item("Seq").Specific.ValidValues.Add("1", "1");
				oForm.Items.Item("Seq").Specific.ValidValues.Add("2", "2");
				oForm.Items.Item("Seq").Specific.Select(0, SAPbouiCOM.BoSearchKey.psk_Index);
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// PS_MM230_Initial_Setting
		/// </summary>
		private void PS_MM230_Initial_Setting()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
			...
			try
			{
				...
				oForm.Items.Item("DocDateTo").Specific.Value = DateTime.Now.ToString("yyyyMM") + "15";
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// PS_MM230_EnableMenus
		/// </summary>
		private void PS_MM230_EnableMenus()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
        ...
        /// <summary>
        /// PS_MM230_SetDocument
        /// </summary>
        /// <param name="oFromDocEntry01"></param>
        private void PS_MM230_SetDocument(string oFromDocEntry01)
        {
            try
            {
        ...
        /// PS_MM230_FormItemEnabled
        /// </summary>
        private void PS_MM230_FormItemEnabled()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// <summary>
		/// PS_MM230_AddMatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_MM230_AddMatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				oForm.Freeze(true);
				if (RowIserted == false)
				{
					oDS_PS_MM230L.InsertRecord(oRow);
				}
				oMat.AddRow();
				oDS_PS_MM230L.Offset = oRow;
				oDS_PS_MM230L.SetValue("U_LineNum", oRow, Convert.ToString(oRow + 1));
		...
		/// PS_MM230_FormClear
		/// </summary>
		private void PS_MM230_FormClear()
		{
			string DocEntry;
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
			...
			try
			{
				DocEntry = dataHelpClass.Get_ReData("AutoKey", "ObjectCode", "ONNM", "'PS_MM230'", "");
		...
		/// <summary>
		/// PS_MM230_DataValidCheck
		/// </summary>
		/// <returns></returns>
		private bool PS_MM230_DataValidCheck()
		{
			...
			try
			{
				...
				oDS_PS_MM230L.RemoveRecord(oDS_PS_MM230L.Size - 1);
				oMat.LoadFromDataSource();
		...
		/// PS_MM230_LoadData
		/// </summary>
		private void PS_MM230_LoadData()
		{
			int i;
			string BPLId;
			string DocDateFr;
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "1")
					{
						...
						{
							if (PS_MM230_DataValidCheck() == false)
							{
								BubbleEvent = false;
								return;
							}
		...
		private void Raise_EVENT_COMBO_SELECT(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				...
				{
					if (pVal.ItemUID == "Seq" && oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
					{
						if (oForm.Items.Item("Seq").Specific.Value.ToString().Trim() == "1")
						{
							oForm.Items.Item("DocDateFr").Specific.Value = oForm.Items.Item("YM").Specific.Value.ToString().Trim() + "01";
							oForm.Items.Item("DocDateTo").Specific.Value = oForm.Items.Item("YM").Specific.Value.ToString().Trim() + "15";
						}
						else if (oForm.Items.Item("Seq").Specific.Value.ToString().Trim() == "2")
						{
							oForm.Items.Item("DocDateFr").Specific.Value = oForm.Items.Item("YM").Specific.Value.ToString().Trim() + "16";
							CDate = codeHelpClass.Left(oForm.Items.Item("YM").Specific.Value.ToString().Trim(), 4) + "-" + codeHelpClass.Right(oForm.Items.Item("YM").Specific.Value.ToString().Trim(), 2) + "-01";
		...
		private void Raise_EVENT_VALIDATE(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				...
				if (pVal.Before_Action == true)
				{
					...
					{
						if (pVal.ItemUID == "YM" && oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
						{
							if (oForm.Items.Item("Seq").Specific.Value.ToString().Trim() == "1")
							{
								oForm.Items.Item("DocDateFr").Specific.Value = oForm.Items.Item("YM").Specific.Value.ToString().Trim() + "01";
		...
		private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.Before_Action == false)
				{
					...
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_MM230H);
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_MM230L);
				}
			}
			catch (Exception ex)
			{
			...
			try
			{
				if (oLastColRow01 > 0)
				{
					...
					else if (pVal.BeforeAction == false)
					{
						...
						if (oMat.RowCount == 0)
						{
							PS_MM230_AddMatrixRow(0, false);
						}
						else
						{
							if (!string.IsNullOrEmpty(oDS_PS_MM230L.GetValue("U_CardCode", oMat.RowCount - 1).ToString().Trim()))
							{
