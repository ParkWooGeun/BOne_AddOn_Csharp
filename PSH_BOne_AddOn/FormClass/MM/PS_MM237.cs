...
namespace PSH_BOne_AddOn
{
	...
	/// 외주입고현황
	/// </summary>
	internal class PS_MM237 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM237.srf");
				...
				oFormUniqueID = "PS_MM237_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM237");
				...
				oForm.Freeze(true);
				PS_MM237_CreateItems();
				PS_MM237_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM237_CreateItems
		/// </summary>
		private void PS_MM237_CreateItems()
		{
			try
			{
		...
		/// PS_MM237_ComboBox_Setting
		/// </summary>
		private void PS_MM237_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				oRecordSet.DoQuery(sQry);
				oForm.Items.Item("Mark").Specific.ValidValues.Add("", "");
				while (!(oRecordSet.EoF))
				{
				...
				oRecordSet.DoQuery(sQry);
				oForm.Items.Item("U_CpBCode").Specific.ValidValues.Add("", "");
				while (!(oRecordSet.EoF))
				{
					oForm.Items.Item("U_CpBCode").Specific.ValidValues.Add(oRecordSet.Fields.Item(0).Value.ToString().Trim(), oRecordSet.Fields.Item(1).Value.ToString().Trim());
					oRecordSet.MoveNext();
				}
				//승인
				oForm.Items.Item("OKYNC").Specific.ValidValues.Add("", "");
				oForm.Items.Item("OKYNC").Specific.ValidValues.Add("Y", "승인");
				oForm.Items.Item("OKYNC").Specific.ValidValues.Add("N", "미승인");
				oForm.Items.Item("OKYNC").Specific.ValidValues.Add("C", "승인취소");
				//정렬구분
				oForm.Items.Item("Gubun").Specific.ValidValues.Add("1", "제품별");
				oForm.Items.Item("Gubun").Specific.ValidValues.Add("2", "일자별");
		...
		/// <summary>
		/// PS_MM237_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		private void PS_MM237_FlushToItemValue(string oUID)
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
		...
		/// <summary>
		/// /PS_MM237_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_MM237_HeaderSpaceLineDel()
		{
			...
			try
			{
				...
				{
					errMessage = "품의일자는 필수입력사항입니다. 확인하세요요.";
					throw new Exception();
				}
		...
		/// <summary>
		/// PS_MM237_Print_Report01 리포트 출력
		/// </summary>
		[STAThread]
		private void PS_MM237_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string Gubun;
			...
			try
			{
				...
				dataPackParameter.Add(new PSH_DataPackClass("@OKYNC", OKYNC));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM237_HeaderSpaceLineDel() == false)
						{
							BubbleEvent = false;
							return;
						}

						System.Threading.Thread thread = new System.Threading.Thread(PS_MM237_Print_Report01);
			...
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "CardCode")
						{
							PS_MM237_FlushToItemValue(pVal.ItemUID);
						}
					}
				}
			}
