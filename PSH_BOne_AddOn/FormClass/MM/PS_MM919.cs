...
namespace PSH_BOne_AddOn
{
	...
	/// 부서별품목별실적집계조회
	/// </summary>
	internal class PS_MM919 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM919.srf");
				...
				oFormUniqueID = "PS_MM919_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM919");
				...
				oForm.Freeze(true);
				PS_MM919_CreateItems();
				PS_MM919_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM919_CreateItems
		/// </summary>
		private void PS_MM919_CreateItems()
		{
			try
			{
		...
		/// PS_MM919_ComboBox_Setting
		/// </summary>
		private void PS_MM919_ComboBox_Setting()
		{
			string BPLID;
			string DocDateF;
			string DocDateT;
		...
		private void Raise_EVENT_VALIDATE(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "DocDateFr" || pVal.ItemUID == "DocDateTo")
					{
						sCount = oForm.Items.Item("OcrCode").Specific.ValidValues.Count;
						sSeq = sCount;
						for (i = 1; i <= sCount; i++)
						{
							oForm.Items.Item("OcrCode").Specific.ValidValues.Remove(sSeq - 1, SAPbouiCOM.BoSearchKey.psk_Index);
                            sSeq -= 1;
                        }
                        BPLID = oForm.Items.Item("BPLId").Specific.Value.ToString().Trim();
						DocDateF = oForm.Items.Item("DocDateFr").Specific.Value.ToString().Trim();
