...
namespace PSH_BOne_AddOn
{
	...
	/// 품목별연간매입집계표
	/// </summary>
	internal class PS_MM928 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM928.srf");
				...
				oFormUniqueID = "PS_MM928_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM928");
				...
				oForm.Freeze(true);
				PS_MM928_CreateItems();
				PS_MM928_ComboBox_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM928_CreateItems
		/// </summary>
		private void PS_MM928_CreateItems()
		{
			try
			{
		...
		/// PS_MM928_ComboBox_Setting
		/// </summary>
		private void PS_MM928_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				oForm.Items.Item("reType").Specific.ValidValues.Add("%", "전체");
				oForm.Items.Item("reType").Specific.ValidValues.Add("10", "자체구매");
				oForm.Items.Item("reType").Specific.ValidValues.Add("20", "통합구매");
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.MessageBox(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message);
		...
		/// <summary>
		/// PS_MM928_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_MM928_HeaderSpaceLineDel()
		{
			...
			try
			{
				...
				{
					errMessage = "기준년도는 필수입력사항입니다. 확인하세요요.";
					throw new Exception();
				}
		...
		/// <summary>
		/// PS_MM928_Print_Report01 리포트 출력
		/// </summary>
		[STAThread]
		private void PS_MM928_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string Purchase;
			...
			try
			{
				...
				ReType = oForm.Items.Item("reType").Specific.Value.ToString().Trim();
				...
				dataPackFormula.Add(new PSH_DataPackClass("@F01", YM.Substring(0, 4) + "-" + YM.Substring(4, 2)));

				if (CardCode == "%")
				{
					dataPackFormula.Add(new PSH_DataPackClass("@F02", "전체"));
				}
				else
				{
					dataPackFormula.Add(new PSH_DataPackClass("@F02", CardName + "[" + CardCode + "]"));
				}

				if (OptBtnValue == "A")
				{
					ReportName = "PS_MM928_01.rpt";
					dataPackFormula.Add(new PSH_DataPackClass("@F03", "A"));
					//Parameter
				...
				{
					ReportName = "PS_MM928_02.rpt";
					dataPackFormula.Add(new PSH_DataPackClass("@F03", "B"));
					//Parameter
					dataPackParameter.Add(new PSH_DataPackClass("@BPLId", BPLId));
					dataPackParameter.Add(new PSH_DataPackClass("@YM", YM));
					dataPackParameter.Add(new PSH_DataPackClass("@ItmBSort", ItmBSort));
					...
					dataPackParameter.Add(new PSH_DataPackClass("@ReType", ReType));
				}
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						if (PS_MM928_HeaderSpaceLineDel() == false)
						{
							BubbleEvent = false;
							return;
						}

						System.Threading.Thread thread = new System.Threading.Thread(PS_MM928_Print_Report01);
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "ItmBSort")
						{
							...
							sQry = "Select U_Code, U_CodeName From [@PSH_ItmMSort] Where U_rCode = '" + oForm.Items.Item("ItmBSort").Specific.Value.ToString().Trim() + "' Order by U_Code";
							oRecordSet.DoQuery(sQry);
