...
namespace PSH_BOne_AddOn
{
	...
	/// 품의발주 대비 가입고 지체현황
	/// </summary>
	internal class PS_MM940 : PSH_BaseClass
	{
		private string oFormUniqueID;
		private SAPbouiCOM.Matrix oMat;
			
		private SAPbouiCOM.DBDataSource oDS_PS_MM940H; //등록헤더
		private SAPbouiCOM.DBDataSource oDS_PS_MM940L; //등록라인
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_MM940.srf");
				...
				oFormUniqueID = "PS_MM940_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_MM940");
				...
				oForm.Freeze(true);
				PS_MM940_CreateItems();
				PS_MM940_ComboBox_Setting();
				PS_MM940_Initial_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_MM940_CreateItems
		/// </summary>
		private void PS_MM940_CreateItems()
		{
			try
			{
				oDS_PS_MM940H = oForm.DataSources.DBDataSources.Item("@PS_USERDS01");
				oDS_PS_MM940L = oForm.DataSources.DBDataSources.Item("@PS_USERDS01");
		...
		/// PS_MM940_ComboBox_Setting
		/// </summary>
		private void PS_MM940_ComboBox_Setting()
		{
			string sQry;
			...
			try
			{
				...
				dataHelpClass.Set_ComboList(oForm.Items.Item("POType").Specific, "SELECT Code, Name From [@PSH_RETYPE]", "", false, false);
		...
		/// PS_MM940_Initial_Setting
		/// </summary>
		private void PS_MM940_Initial_Setting()
		{
			try
			{
				oForm.Items.Item("Chk01").Specific.Checked = true; //체크박스 설정
		...
		/// <summary>
		/// PS_MM940_AddMatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_MM940_AddMatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				oForm.Freeze(true);
				if (RowIserted == false)
				{
					oDS_PS_MM940L.InsertRecord(oRow);
				}
				oMat.AddRow();
				oDS_PS_MM940L.Offset = oRow;
		...
		private void PS_MM940_MTX01()
		{
			...
			string POType;   //품의형태
			string Purchase; //품의구분
			string FrDt;     //납기일시작
			string ToDt;     //납기일종료
			string Chk01;    //지체건만 조회
			...
			try
			{
				...
				sQry = "EXEC PS_MM940_01 '" + BPLID + "','" + CardCode + "','" + ItemCode + "','" + POType + "','" + Purchase + "','" + FrDt + "','" + ToDt + "','" + Chk01 + "'";
				oRecordSet.DoQuery(sQry);
				...
				for (loopCount = 0; loopCount <= oRecordSet.RecordCount - 1; loopCount++)
				{
					if (loopCount != 0)
					{
						oDS_PS_MM940L.InsertRecord(loopCount);
					}

					oDS_PS_MM940L.Offset = loopCount;
					oDS_PS_MM940L.SetValue("U_LineNum", loopCount, Convert.ToString(loopCount + 1)); //라인번호
					oDS_PS_MM940L.SetValue("U_ColReg10", loopCount, oRecordSet.Fields.Item("DocEntry").Value.ToString().Trim());    //품의문서번호
					oDS_PS_MM940L.SetValue("U_ColReg01", loopCount, oRecordSet.Fields.Item("CardCode").Value.ToString().Trim());    //업체코드
					oDS_PS_MM940L.SetValue("U_ColReg02", loopCount, oRecordSet.Fields.Item("CardName").Value.ToString().Trim());    //업체명
					oDS_PS_MM940L.SetValue("U_ColReg03", loopCount, oRecordSet.Fields.Item("ItemCode").Value.ToString().Trim());    //품목코드(작번)
					oDS_PS_MM940L.SetValue("U_ColReg04", loopCount, oRecordSet.Fields.Item("ItemName").Value.ToString().Trim());    //품목명
					oDS_PS_MM940L.SetValue("U_ColReg05", loopCount, oRecordSet.Fields.Item("Spec").Value.ToString().Trim());        //규격
					oDS_PS_MM940L.SetValue("U_ColReg06", loopCount, oRecordSet.Fields.Item("Unit_NM").Value.ToString().Trim());     //단위
					oDS_PS_MM940L.SetValue("U_ColReg07", loopCount, oRecordSet.Fields.Item("POType_NM").Value.ToString().Trim());   //품의형태(콤보박스를 이용하지 않고 명을 그대로 출력)
					oDS_PS_MM940L.SetValue("U_ColReg08", loopCount, oRecordSet.Fields.Item("Purchase_NM").Value.ToString().Trim()); //품의구분(콤보박스를 이용하지 않고 명을 그대로 출력)
					oDS_PS_MM940L.SetValue("U_ColQty01", loopCount, oRecordSet.Fields.Item("PO_Qty").Value.ToString().Trim());      //품의수량
					oDS_PS_MM940L.SetValue("U_ColQty02", loopCount, oRecordSet.Fields.Item("In_Qty").Value.ToString().Trim());      //가입고수량
					oDS_PS_MM940L.SetValue("U_ColSum01", loopCount, oRecordSet.Fields.Item("PO_Amt").Value.ToString().Trim());      //품의금액
					oDS_PS_MM940L.SetValue("U_ColSum02", loopCount, oRecordSet.Fields.Item("In_Amt").Value.ToString().Trim());      //가입고금액
					oDS_PS_MM940L.SetValue("U_ColDt01", loopCount, Convert.ToDateTime(oRecordSet.Fields.Item("PO_Dt").Value.ToString().Trim()).ToString("yyyyMMdd")); //품의납품일
					oDS_PS_MM940L.SetValue("U_ColDt02", loopCount, Convert.ToDateTime(oRecordSet.Fields.Item("In_Dt").Value.ToString().Trim()).ToString("yyyyMMdd")); //가입고납품일
					oDS_PS_MM940L.SetValue("U_ColNum01", loopCount, oRecordSet.Fields.Item("Delay_Dt").Value.ToString().Trim());    //지체일수
					oDS_PS_MM940L.SetValue("U_ColSum03", loopCount, oRecordSet.Fields.Item("Delay_Amt").Value.ToString().Trim());   //지체대상금액
					oRecordSet.MoveNext();
		...
		/// <summary>
		/// PS_MM940_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_MM940_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string BPLID;    //사업장
			...
			try
			{
				...
				WinTitle = "[PS_MM940] 레포트";
				ReportName = "PS_MM940_01.rpt";
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					else if (pVal.ItemUID == "Btn_Print")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_MM940_Print_Report01);
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Mat01")
					{
						...
						{
							PS_MM030 oTempClass = new PS_MM030();
		...
		private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.Before_Action == false)
				{
					...
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_MM940H);
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_MM940L);
				}
			}
			catch (Exception ex)
			{
				...
				if (pVal.BeforeAction == true)
				{
					switch (pVal.MenuUID)
					{
						...
						case "1293": //행삭제
							break;
						case "7169": //엑셀 내보내기
							PS_MM940_AddMatrixRow(oMat.VisualRowCount, false);
							break;
					}
				}
				else if (pVal.BeforeAction == false)
				{
					switch (pVal.MenuUID)
					{
						...
						case "1293": //행삭제
							break;
						case "7169": //엑셀 내보내기
							oDS_PS_MM940L.RemoveRecord(oDS_PS_MM940L.Size - 1);
							oMat.LoadFromDataSource();
							break;
					}
				}
