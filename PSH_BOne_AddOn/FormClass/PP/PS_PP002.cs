...
namespace PSH_BOne_AddOn
{
    ...
    internal class PS_PP002 : PSH_BaseClass
    {
        ...
        /// 화면 Item 생성
        /// </summary>
        private void PS_PP002_CreateItems()
        {
            try
            {
                oDS_PS_PP002H = oForm.DataSources.DBDataSources.Item("@PS_PP002H");
                oDS_PS_PP002L = oForm.DataSources.DBDataSources.Item("@PS_PP002L");
        ...
        /// Combobox 설정
        /// </summary>
        private void PS_PP002_SetComboBox()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();

            try
            {
                dataHelpClass.Set_ComboList(oForm.Items.Item("BigCode").Specific, "SELECT U_Minor,U_CdName FROM [@PS_SY001L] WHERE Code = 'Q001' order by LineId", "1", false, false);
            }
            catch (Exception ex)
            {
                PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
        ...
        /// 각 모드에 따른 아이템설정
        /// </summary>
        private void PS_PP002_EnableFormItem()
        {
            try
            {
                oForm.Freeze(true);
        ...
        /// DocEntry 초기화
        /// </summary>
        private void PS_PP002_SetDocEntry()
        {
            string DocEntry;
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            ...
            try
            {
                DocEntry = dataHelpClass.Get_ReData("AutoKey", "ObjectCode", "ONNM", "'PS_PP002'", "");
        ...
        private void PS_PP002_AddMatrixRow(int oRow, bool RowIserted)
        {
            try
            {
                oForm.Freeze(true);
        ...
        private void PS_PP002_SetDocument(string oFormDocEntry)
        {
            try
            {
                ...
                {
                    PS_PP002_EnableFormItem();
                    PS_PP002_AddMatrixRow(0, true);
                }
                else
                {
                }
        ...
        /// </summary>
        /// <returns></returns>
        private bool PS_PP002_CheckDataValid()
        {
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                {
                    if (Convert.ToInt16(dataHelpClass.GetValue("SELECT COUNT(*) FROM [@PS_PP002H] WHERE U_BigCode = '" + oForm.Items.Item("BigCode").Specific.Selected.Value + "'", 0, 1)) > 0)
                    {
            ...
            try
            {
                if (pVal.BeforeAction == true)
                {
                    if (pVal.ItemUID == "1")
                    {
                        if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                        {
                            if (PS_PP002_CheckDataValid() == false)
                            {
                                BubbleEvent = false;
                                return;
                            }
                ...
                else if (pVal.BeforeAction == false)
                {
                    if (pVal.ItemUID == "1")
                    {
                        if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                        {
                            ...
                            {
                                PS_PP002_EnableFormItem();
                                PS_PP002_AddMatrixRow(oMat01.RowCount, true);
                            }
                        }
                        else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                        {
                    ...
                    if (pVal.ItemChanged == true)
                    {
                        if (pVal.ItemUID == "Mat01")
                        {
                            if (pVal.ColUID == "MidCode")
                            {
                                ...
                                if (oMat01.RowCount == pVal.Row && !string.IsNullOrEmpty(oDS_PS_PP002L.GetValue("U_" + pVal.ColUID, pVal.Row - 1).ToString().Trim()))
                                {
                                    PS_PP002_AddMatrixRow(pVal.Row, false);
                                }
        ...
        private void Raise_EVENT_MATRIX_LOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                {
                    PS_PP002_EnableFormItem();
                    PS_PP002_AddMatrixRow(oMat01.VisualRowCount, false);
                }
            }
            catch (Exception ex)
            {
        ...
        private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                else if (pVal.Before_Action == false)
                {
                    ...
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_PP002H);
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_PP002L);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (oLastColRow01 > 0)
                {
                    ...
                    else if (pVal.BeforeAction == false)
                    {
                        ...
                        {
                            PS_PP002_AddMatrixRow(0, false);
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(oDS_PS_PP002L.GetValue("U_MidCode", oMat01.RowCount - 1).ToString().Trim()))
                            {
                                PS_PP002_AddMatrixRow(oMat01.RowCount, false);
                            }
                        }
                    }
                }
