	...
	internal class PS_PP011 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				...
				PS_PP011_CreateItems();
				PS_PP011_SetDocument(oFormDocEntry);
		...
		/// PS_PP011_CreateItems
		/// </summary>
		private void PS_PP011_CreateItems()
		{
			try
			{
		...
		/// <summary>
		/// PS_PP011_SetDocument
		/// </summary>
		/// <param name="oFormDocEntry"></param>
		private void PS_PP011_SetDocument(string oFormDocEntry)
		{
			try
			{
		...
		/// <summary>
		/// PS_PP011_AddMatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_PP011_AddMatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				oForm.Freeze(true);
		...
		/// PS_PP011_EnableFormItem
		/// </summary>
		private void PS_PP011_EnableFormItem()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// <summary>
		/// PS_PP011_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP011_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
				...
				switch (oUID)
				{
					...
					case "Mat01":
						if (oCol == "ItemCode")
						{
							...
							{
								oMat.FlushToDataSource();
								PS_PP011_AddMatrixRow(oMat.RowCount, false);
		...
		/// <summary>
		/// PS_PP011_DelHeaderSpaceLine
		/// </summary>
		/// <returns></returns>
		private bool PS_PP011_DelHeaderSpaceLine()
		{
		...
		/// <summary>
		/// PS_PP011_DelMatrixSpaceLine
		/// </summary>
		/// <returns></returns>
		private bool PS_PP011_DelMatrixSpaceLine()
		{
			bool functionReturnValue = false;
			string errMessage = string.Empty;
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "1")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							if (PS_PP011_DelHeaderSpaceLine() == false)
							{
								BubbleEvent = false;
								return;
							}

							if (PS_PP011_DelMatrixSpaceLine() == false)
							{
								BubbleEvent = false;
								return;
							}
				...
				else if (pVal.BeforeAction == false)
				{
					...
					if (pVal.BeforeAction == false && pVal.ItemChanged == true)
					{
						...
						{
							PS_PP011_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
						}
					}
				}
			}
		...
		private void Raise_EVENT_MATRIX_LOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				{
					PS_PP011_AddMatrixRow(oMat.VisualRowCount, false);
					PS_PP011_EnableFormItem();
				}
			}
			catch (Exception ex)
