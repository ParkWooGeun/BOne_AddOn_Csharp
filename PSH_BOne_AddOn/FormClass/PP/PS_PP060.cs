	...
	internal class PS_PP060 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				...
				PS_PP060_SetComboBox();
				PS_PP060_AddMatrixRow(0, true);
				PS_PP060_LoadCaption();
				PS_PP060_EnableFormItem();
		...
		private void PS_PP060_CreateItems()
		{
			try
			{
				...
				oMat.Columns.Item("DocNum").Visible = false;
				...
				//공정구분
				oForm.DataSources.UserDataSources.Add("CpGbn", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 5);
				oForm.Items.Item("CpGbn").Specific.DataBind.SetBound(true, "", "CpGbn");
				...
				//금형코드(코드)
				oForm.DataSources.UserDataSources.Add("ItmBsort", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 50);
				oForm.Items.Item("ItmBsort").Specific.DataBind.SetBound(true, "", "ItmBsort");

				//금형코드(명)
				oForm.DataSources.UserDataSources.Add("ItmName", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 100);
				oForm.Items.Item("ItmName").Specific.DataBind.SetBound(true, "", "ItmName");

				//사원코드(코드)
				oForm.DataSources.UserDataSources.Add("CntcCode", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 50);
		...
		/// PS_PP060_SetComboBox
		/// </summary>
		private void PS_PP060_SetComboBox()
		{
			string sQry;
			...
			try
			{
				...
				dataHelpClass.Set_ComboList(oForm.Items.Item("WorkGbn").Specific, "select Code, Name from [@PSH_ITMBSORT] Where U_PudYN = 'Y' order by Code", "", false, false);
		...
		/// <summary>
		/// PS_PP060_AddMatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_PP060_AddMatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				if (RowIserted == false)
		...
		/// PS_PP060_LoadCaption
		/// </summary>
		private void PS_PP060_LoadCaption()
		{
			try
			{
		...
		/// PS_PP060_EnableFormItem
		/// </summary>
		private void PS_PP060_EnableFormItem()
		{
			try
			{
				if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
		...
		/// <summary>
		/// PS_PP060_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP060_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
				...
				switch (oUID)
				{
					...
					case "Mat01": //메트릭스
						if (oCol == "CntcCode") //사번코드
						{
							...
							{
								PS_PP060_AddMatrixRow(oRow, false);
							}
		...
		/// 조회데이타 가져오기
		/// </summary>
		private void PS_PP060_LoadData()
		{
			int i;
			string sQry;
			string errMessage = string.Empty;
		...
		/// PS_PP060_DeleteData
		/// </summary>
		private void PS_PP060_DeleteData()
		{
			int i;
			string sQry;
			string ItmRegYN;
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Btn_save")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							if (PS_PP060_DelHeaderSpaceLine() == false)
							{
								BubbleEvent = false;
								return;
							}
							if (PS_PP060_DelMatrixSpaceLine() == false)
							{
								BubbleEvent = false;
								return;
							}
							if (PS_PP060_AddData(ref pVal) == false)
							{
								BubbleEvent = false;
								return;
							}
						...
						{
							if (PS_PP060_UpdateData(ref pVal) == false)
							{
								BubbleEvent = false;
								return;
							}
					...
					else if (pVal.ItemUID == "Btn_del")
					{
						...
						if (PSH_Globals.SBO_Application.MessageBox("삭제 후에는 복구가 불가능합니다. 삭제하시겠습니까?" + (char)13 + "※소요자재가 등록된 행은 삭제가 불가능합니다. 소요자재 정보를 먼저 삭제하세요.", 1, "예", "아니오") == 1)
						{
							PS_PP060_DeleteData();
							PS_PP060_LoadData();
						}
						...
						else
						{
							...
							switch (oForm.Items.Item("CpGbn").Specific.Value.ToString().Trim())
							{
								case "10": //v-mill
									SCpCode = "CP50101";
									break;
								case "20": // 콤보박스에 값을 지정 멀티게이지-외경연삭일때는 FRM공정을 SD380
									SCpCode = "CP50104";
									break;
								case "30": // 콤보박스에 값을 지정 멀티게이지-포장일때는 FRM공정을 SD380
									SCpCode = "CP50107";
									break;
								case "40": //휘팅바렐
									SCpCode = "CP30112";
							...
							if (oForm.Items.Item("CpGbn").Specific.Value.ToString().Trim() != "60" 
                                && oForm.Items.Item("CpGbn").Specific.Value.ToString().Trim() != "70" 
			...
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "CntcCode")
						{
							PS_PP060_FlushToItemValue(pVal.ItemUID, 0, "");
						}
						else if (pVal.ItemUID == "Mat01")
						{
							if (pVal.ColUID == "CntcCode")
							{
								PS_PP060_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
							}
