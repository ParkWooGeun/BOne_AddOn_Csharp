...
namespace PSH_BOne_AddOn
{
	...
	/// 작업실적입력현황(현장)
	/// </summary>
	internal class PS_PP230 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP230.srf");
				...
				oFormUniqueID = "PS_PP230_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP230");
				...
				oForm.Freeze(true);
				PS_PP230_CreateItems();
				PS_PP230_SetComboBox();
				PS_PP230_Initialize();
		...
		/// PS_PP230_CreateItems
		/// </summary>
		private void PS_PP230_CreateItems()
		{
			try
			{
		...
		/// PS_PP230_SetComboBox
		/// </summary>
		private void PS_PP230_SetComboBox()
		{
			string sQry;
		...
		/// PS_PP230_Initialize
		/// </summary>
		private void PS_PP230_Initialize()
		{
		...
		/// <summary>
		/// PS_PP230_PrintReport
		/// </summary>
		[STAThread]
		private void PS_PP230_PrintReport()
		{
			string sQry;
			string WinTitle;
			string ReportName;
			...
			try
			{
				...
				WinTitle = "[PS_PP230_01] 작업실적입력현황(현장)";
				ReportName = "PS_PP230_01.RPT";
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						System.Threading.Thread thread = new System.Threading.Thread(PS_PP230_PrintReport);
			...
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							sQry = "SELECT U_CpName FROM [@PS_PP001H] AS A Inner Join [@PS_PP001L] AS B  On A.Code = B.Code WHERE A.U_CpBCode = 'CP501' And B.U_CpCode =  '" + oForm.Items.Item("CpCode").Specific.Value.ToString().Trim() + "'";
							oRecordSet.DoQuery(sQry);
							oForm.Items.Item("CpName").Specific.String = oRecordSet.Fields.Item("U_CpName").Value.ToString().Trim();
						}
						if (pVal.ItemUID == "WorkCode")
						{
							sQry = "SELECT U_WorkName FROM [@PS_PP040M] WHERE U_WorkCode =  '" + oForm.Items.Item("WorkCode").Specific.Value.ToString().Trim() + "'";
							oRecordSet.DoQuery(sQry);
							oForm.Items.Item("WorkName").Specific.String = oRecordSet.Fields.Item("U_WorkName").Value.ToString().Trim();
						}
						if (pVal.ItemUID == "MachCode")
						{
							sQry = "SELECT U_MachName FROM [@PS_PP130H] WHERE U_MachCode  =  '" + oForm.Items.Item("MachCode").Specific.Value.ToString().Trim() + "'";
							oRecordSet.DoQuery(sQry);
							oForm.Items.Item("MachName").Specific.String = oRecordSet.Fields.Item("U_MachName").Value.ToString().Trim();
						}
					}
				}
			}
