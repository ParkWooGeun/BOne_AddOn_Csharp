	...
	internal class PS_PP282 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFromDocEntry01)
		{
			...
			try
			{
				...
				PS_PP282_ComboBox_Setting();
				PS_PP282_EnableMenus();
				PS_PP282_SetDocument(oFromDocEntry01);
		...
		/// PS_PP282_ComboBox_Setting
		/// </summary>
		private void PS_PP282_ComboBox_Setting()
		{
			string sQry;
		...
		/// </summary>
		/// <param name="oFromDocEntry01"></param>
		private void PS_PP282_SetDocument(string oFromDocEntry01)
		{
			try
			{
				if (string.IsNullOrEmpty(oFromDocEntry01))
				{
					PS_PP282_FormItemEnabled();
					PS_PP282_AddMatrixRow(0, true); //UDO방식일때
				}
				else
				{
				}
		...
		/// PS_PP282_FormItemEnabled
		/// </summary>
		private void PS_PP282_FormItemEnabled()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
		...
		/// PS_PP282_FormClear
		/// </summary>
		private void PS_PP282_FormClear()
		{
			string DocEntry;
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
		...
		/// <summary>
		/// PS_PP282_DataValidCheck
		/// </summary>
		/// <returns></returns>
		private bool PS_PP282_DataValidCheck()
		{
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "1")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							if (PS_PP282_DataValidCheck() == false)
							{
								BubbleEvent = false;
								return;
							}
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
						...
						&& (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE))
					{
						...
						sQry = sQry + "And U_Code2 = '" + oForm.Items.Item("Code2").Specific.Value.ToString().Trim() + "'";
						oRecordSet.DoQuery(sQry);
		...
		private void Raise_EVENT_MATRIX_LOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				{
					PS_PP282_FormItemEnabled();
					PS_PP282_AddMatrixRow(oMat.VisualRowCount, false); //UDO방식
				}
			}
			catch (Exception ex)
			{
				...
				else if (pVal.BeforeAction == false)
				{
					switch (pVal.MenuUID)
					{
						...
						case "1293": //행삭제
							...
							break;
						case "1281": //찾기
							PS_PP282_FormItemEnabled(); //UDO방식
							oForm.Items.Item("DocEntry").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
							break;
						case "1282": //추가
							PS_PP282_FormItemEnabled(); //UDO방식
							PS_PP282_AddMatrixRow(0, true); //UDO방식
							oForm.Items.Item("Code1").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
							break;
						case "1288": //레코드이동(최초)
						case "1289": //레코드이동(이전)
