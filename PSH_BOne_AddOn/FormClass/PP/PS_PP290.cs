...
namespace PSH_BOne_AddOn
{
	...
	/// 설비정비현황출력
	/// </summary>
	internal class PS_PP290 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP290.srf");
				...
				oFormUniqueID = "PS_PP290_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP290");
				...
				oForm.Freeze(true);
				PS_PP290_CreateItems();
				PS_PP290_SetComboBox();
				PS_PP290_Initialize();
		...
		/// PS_PP290_CreateItems
		/// </summary>
		private void PS_PP290_CreateItems()
		{
			try
			{
		...
		/// PS_PP290_SetComboBox
		/// </summary>
		private void PS_PP290_SetComboBox()
		{
			string sQry;
			...
			try
			{
				...
				oForm.Items.Item("ClsCode").Specific.ValidValues.Add("10", "동력");
				oForm.Items.Item("ClsCode").Specific.ValidValues.Add("20", "공무");
				oForm.Items.Item("ClsCode").Specific.ValidValues.Add("90", "공통");
				oForm.Items.Item("ClsCode").Specific.Select("%", SAPbouiCOM.BoSearchKey.psk_ByValue);

				//작업구분
				oForm.Items.Item("WorkDiv").Specific.ValidValues.Add("%", "전체");
				dataHelpClass.Set_ComboList(oForm.Items.Item("WorkDiv").Specific, "SELECT U_Minor, U_CdName FROM [@PS_SY001L] WHERE CODE = 'P220' order by U_Seq", "", false, false);
				oForm.Items.Item("WorkDiv").Specific.Select("%", SAPbouiCOM.BoSearchKey.psk_ByValue);
		...
		/// PS_PP290_Initialize
		/// </summary>
		private void PS_PP290_Initialize()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
		...
		/// <summary>
		/// PS_PP290_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP290_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
			...
			try
			{
				switch (oUID)
				{
					case "Code1":
						...
						oRecordSet.DoQuery(sQry);
						oForm.Items.Item("Code1NM").Specific.String = oRecordSet.Fields.Item("U_Name1").Value.ToString().Trim();
						break;
				}
			}
			catch (Exception ex)
		...
		private void PS_PP290_PrintReport()
		{
			...
			try
			{
				...
				WorkDiv   = oForm.Items.Item("WorkDiv").Specific.Value.ToString().Trim();
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						System.Threading.Thread thread = new System.Threading.Thread(PS_PP290_PrintReport);
		...
		private void Raise_EVENT_KEY_DOWN(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					dataHelpClass.ActiveUserDefineValue(ref oForm, ref pVal, ref BubbleEvent, "Code1", "");
				}
				else if (pVal.BeforeAction == false)
				{
				}
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							PS_PP290_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
						}
					}
				}
				else if (pVal.BeforeAction == false)
