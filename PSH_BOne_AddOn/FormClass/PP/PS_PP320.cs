...
namespace PSH_BOne_AddOn
{
	...
	/// 부품주문번호별수주잔량현황
	/// </summary>
	internal class PS_PP320 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP320.srf");
				...
				oFormUniqueID = "PS_PP320_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP320");
		...
		/// PS_PP320_CreateItems
		/// </summary>
		private void PS_PP320_CreateItems()
		{
			try
			{
				...
				oForm.DataSources.UserDataSources.Item("DocDateFr").Value = Convert.ToString(DateTime.Now.Year - 1) + "0101";
		...
		/// <summary>
		/// PS_PP320_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP320_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
			...
			try
			{
				switch (oUID)
				{
					case "ItmMsort":
						...
						oForm.Items.Item("MName").Specific.String = oRecordSet.Fields.Item("U_CodeName").Value.ToString().Trim();
						break;
				}
			}
			catch (Exception ex)
		...
		/// <summary>
		/// PS_PP320_PrintReport
		/// </summary>
		[STAThread]
		private void PS_PP320_PrintReport()
		{
			string WinTitle;
			string ReportName;
			string DocDateFr;
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						System.Threading.Thread thread = new System.Threading.Thread(PS_PP320_PrintReport);
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							PS_PP320_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
						}
					}
				}
				else if (pVal.BeforeAction == false)
