...
namespace PSH_BOne_AddOn
{
	...
	/// 생산실적
	/// </summary>
	internal class PS_PP520 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP520.srf");
				...
				oFormUniqueID = "PS_PP520_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP520");
				...
				oForm.Freeze(true);
				PS_PP520_CreateItems();
				PS_PP520_SetComboBox();
				PS_PP520_Initialize();
		...
		/// PS_PP520_CreateItems
		/// </summary>
		private void PS_PP520_CreateItems()
		{
			try
			{
				//사업장
				...
				//품목구분
				oForm.DataSources.UserDataSources.Add("Pumtxt", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 5);
				oForm.Items.Item("Pumtxt").Specific.DataBind.SetBound(true, "", "Pumtxt");
				...
				//자체/외주구분
				oForm.DataSources.UserDataSources.Add("InOutGbn", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 5);
				...
				oForm.Items.Item("Rad03").Specific.DataBind.SetBound(true, "", "RadioBtn");
				...
				oForm.Items.Item("Rad04").Specific.DataBind.SetBound(true, "", "RadioBtn");
		...
		/// PS_PP520_SetComboBox
		/// </summary>
		private void PS_PP520_SetComboBox()
		{
			string sQry;
		...
		/// PS_PP520_Initialize
		/// </summary>
		private void PS_PP520_Initialize()
		{
			...
			try
			{
				...
				oForm.Items.Item("Pumtxt").Specific.ValidValues.Add("", "");
				dataHelpClass.Set_ComboList(oForm.Items.Item("Pumtxt").Specific, "SELECT U_Minor, U_CdName FROM [@PS_SY001L] WHERE Code = 'S002' ORDER BY Code", "", false, false);
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
		...
		/// <summary>
		/// PS_PP520_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP520_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
			SAPbobsCOM.Recordset oRecordSet = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
		...
		/// <summary>
		/// PS_PP520_PrintReport
		/// </summary>
		[STAThread]
		private void PS_PP520_PrintReport()
		{
			string WinTitle;
			...
			try
			{
				...
				Pumtxt    = oForm.Items.Item("Pumtxt").Specific.Value.ToString().Trim();
				...
				if (string.IsNullOrEmpty(Pumtxt))
				{
					Pumtxt = "%";
				}
				...
				if (oForm.DataSources.UserDataSources.Item("RadioBtn").Value == "D")
				{
					ReportName = "PS_PP520_01.RPT";
				}
				else if (oForm.DataSources.UserDataSources.Item("RadioBtn").Value == "A")
				{
				...
				else if (oForm.DataSources.UserDataSources.Item("RadioBtn").Value == "C")
				{
				...
				// Parameter
				if (oForm.DataSources.UserDataSources.Item("RadioBtn").Value == "B")
				{
					dataPackParameter.Add(new PSH_DataPackClass("@BPLId", BPLID));
					dataPackParameter.Add(new PSH_DataPackClass("@Minor", Pumtxt));
					...
					dataPackParameter.Add(new PSH_DataPackClass("@InOutGbn", InOutGbn));
				}
				else
                {
					dataPackParameter.Add(new PSH_DataPackClass("@BPLId", BPLID));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						System.Threading.Thread thread = new System.Threading.Thread(PS_PP520_PrintReport);
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							PS_PP520_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
						}
					}
				}
				else if (pVal.BeforeAction == false)
