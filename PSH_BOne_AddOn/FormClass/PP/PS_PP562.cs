...
namespace PSH_BOne_AddOn
{
	...
	internal class PS_PP562 : PSH_BaseClass
	{
		...
		/// PS_PP562_ComboBox_Setting
		/// </summary>
		private void PS_PP562_ComboBox_Setting()
		{
			string sQry;
		...
		/// PS_PP562_FormResize
		/// </summary>
		private void PS_PP562_FormResize()
		{
			try
			{
				oMat.AutoResizeColumns();
		...
		/// <summary>
		/// PS_PP562_Add_MatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_PP562_Add_MatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				if (RowIserted == false)
		...
		/// <summary>
		/// PS_PP562_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_PP562_HeaderSpaceLineDel()
		{
		...
		/// PS_PP562_FormReset
		/// </summary>
		private void PS_PP562_FormReset()
		{
			try
			{
				oForm.Freeze(true);
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "BtnAdd")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							if (PS_PP562_HeaderSpaceLineDel() == false)
							{
								BubbleEvent = false;
								return;
							}
				...
				if (pVal.BeforeAction == true)
				{
					switch (pVal.MenuUID)
					{
						...
						case "7169": //엑셀 내보내기
							...
							oForm.Freeze(true);
							PS_PP562_Add_MatrixRow(oMat.VisualRowCount, false);
							oForm.Freeze(false);
							break;
					}
				}
