...
namespace PSH_BOne_AddOn
{
	...
	/// 장비별 견적금액 수정관리(영업)
	/// </summary>
	internal class PS_PP563 : PSH_BaseClass
	{
		private string oFormUniqueID;
		private SAPbouiCOM.Matrix oMat;
		private SAPbouiCOM.DBDataSource oDS_PS_PP563B; //등록라인
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP563.srf");
				...
				oFormUniqueID = "PS_PP563_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP563");
				...
				PS_PP563_CreateItems();
				PS_PP563_ComboBox_Setting();
				PS_PP563_FormResize();
				PS_PP563_LoadCaption();
		...
		/// PS_PP563_CreateItems
		/// </summary>
		private void PS_PP563_CreateItems()
		{
			try
			{
				oDS_PS_PP563B = oForm.DataSources.DBDataSources.Item("@PS_USERDS01");
		...
		/// PS_PP563_ComboBox_Setting
		/// </summary>
		private void PS_PP563_ComboBox_Setting()
		{
			string sQry;
		...
		/// PS_PP563_FormResize
		/// </summary>
		private void PS_PP563_FormResize()
		{
			try
			{
				oMat.AutoResizeColumns();
		...
		/// PS_PP563_LoadCaption
		/// </summary>
		private void PS_PP563_LoadCaption()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// <summary>
		/// PS_PP563_Add_MatrixRow
		/// </summary>
		/// <param name="oRow"></param>
		/// <param name="RowIserted"></param>
		private void PS_PP563_Add_MatrixRow(int oRow, bool RowIserted)
		{
			try
			{
				if (RowIserted == false)
				{
					oDS_PS_PP563B.InsertRecord(oRow);
				}

				oMat.AddRow();
				oDS_PS_PP563B.Offset = oRow;
				oDS_PS_PP563B.SetValue("U_LineNum", oRow, Convert.ToString(oRow + 1));
		...
		/// <summary>
		/// PS_PP563_HeaderSpaceLineDel
		/// </summary>
		/// <returns></returns>
		private bool PS_PP563_HeaderSpaceLineDel()
		{
		...
		/// <summary>
		/// PS_PP563_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_PP563_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			string sQry;
		...
		/// PS_PP563_FormReset
		/// </summary>
		private void PS_PP563_FormReset()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// <summary>
		/// PS_PP563_DeleteData  기본정보 삭제(Delete 사용 안함)
		/// </summary>
		private void PS_PP563_DeleteData()
		{
			string sQry;
			string errMessage = String.Empty;
			string DocEntry;
			...
			try
			{
				if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
				{
					...
					{
						sQry = "EXEC PS_PP563_04 '" + DocEntry + "'";
						oRecordSet.DoQuery(sQry);
					}
				}
		...
		/// <summary>
		/// PS_PP563_UpdateData  기본정보를 수정(Update 사용 안함)
		/// </summary>
		/// <returns></returns>
		private bool PS_PP563_UpdateData()
		{
			bool functionReturnValue = false;
		...
		/// <summary>
		/// PS_PP563_AddData 데이터 INSERT
		/// </summary>
		/// <returns></returns>
		private bool PS_PP563_AddData()
		{
			bool functionReturnValue = false;
			string sQry;
		...
		/// <summary>
		/// PS_PP563_MTX01 데이터 조회
		/// </summary>
		private void PS_PP563_MTX01()
		{
			int i;
			string sQry;
			string errMessage = string.Empty;
			...
			try
			{
				...
				{
					if (i + 1 > oDS_PS_PP563B.Size)
					{
						oDS_PS_PP563B.InsertRecord(i);
					}

					oMat.AddRow();
					oDS_PS_PP563B.Offset = i;

					oDS_PS_PP563B.SetValue("U_LineNum", i, Convert.ToString(i + 1));
					oDS_PS_PP563B.SetValue("U_ColReg01", i, oRecordSet.Fields.Item("AmtCls").Value.ToString().Trim());      //비용구분
					oDS_PS_PP563B.SetValue("U_ColReg02", i, oRecordSet.Fields.Item("OrdNum").Value.ToString().Trim());      //작지번호
					oDS_PS_PP563B.SetValue("U_ColReg03", i, oRecordSet.Fields.Item("OrdSub1").Value.ToString().Trim());     //서브작번1
					oDS_PS_PP563B.SetValue("U_ColReg04", i, oRecordSet.Fields.Item("OrdSub2").Value.ToString().Trim());     //서브작번2
					oDS_PS_PP563B.SetValue("U_ColReg05", i, oRecordSet.Fields.Item("OrdName").Value.ToString().Trim());     //작지명
					oDS_PS_PP563B.SetValue("U_ColSum01", i, oRecordSet.Fields.Item("Amount").Value.ToString().Trim());      //목표금액
					oDS_PS_PP563B.SetValue("U_ColReg07", i, oRecordSet.Fields.Item("Comment").Value.ToString().Trim());     //비고
					oDS_PS_PP563B.SetValue("U_ColReg08", i, oRecordSet.Fields.Item("UpdateUser").Value.ToString().Trim());  //수정자
					oDS_PS_PP563B.SetValue("U_ColReg09", i, oRecordSet.Fields.Item("UpdateDate").Value.ToString().Trim());  //수정일
					oDS_PS_PP563B.SetValue("U_ColReg10", i, oRecordSet.Fields.Item("UpdateTime").Value.ToString().Trim());  //수정시간
					oDS_PS_PP563B.SetValue("U_ColReg11", i, oRecordSet.Fields.Item("BaseDL").Value.ToString().Trim());      //기준문서
					oDS_PS_PP563B.SetValue("U_ColReg12", i, oRecordSet.Fields.Item("MainOrd").Value.ToString().Trim());     //메인작번
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "BtnAdd")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							if (PS_PP563_HeaderSpaceLineDel() == false)
							{
								BubbleEvent = false;
								return;
							}
							if (PS_PP563_AddData() == false)
							{
								BubbleEvent = false;
								return;
							}
		...
		private void Raise_EVENT_COMBO_SELECT(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				{
					PS_PP563_FlushToItemValue(pVal.ItemUID, 0, "");
				}
			}
			catch (Exception ex)
			{
		...
		private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					...
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_PP563B);
				}
			}
			catch (Exception ex)
			{
				...
				if (pVal.BeforeAction == true)
				{
					switch (pVal.MenuUID)
					{
						...
						case "7169": //엑셀 내보내기
							...
							oForm.Freeze(true);
							PS_PP563_Add_MatrixRow(oMat.VisualRowCount, false);
							oForm.Freeze(false);
							break;
					}
				}
				else if (pVal.BeforeAction == false)
				{
					switch (pVal.MenuUID)
					{
						...
						case "7169": //엑셀 내보내기
									 //엑셀 내보내기 이후 처리
							oForm.Freeze(true);
							oDS_PS_PP563B.RemoveRecord(oDS_PS_PP563B.Size - 1);
							oMat.LoadFromDataSource();
							oForm.Freeze(false);
							break;
					}
