...
namespace PSH_BOne_AddOn
{
	...
	/// 원인별 불량현황 (멀티)
	/// </summary>
	internal class PS_PP662 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP662.srf");
				...
				oFormUniqueID = "PS_PP662_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP662");
				...
				oForm.Freeze(true);
				PS_PP662_CreateItems();
				PS_PP662_SetComboBox();
				PS_PP662_Initialize();
		...
		/// PS_PP662_CreateItems
		/// </summary>
		private void PS_PP662_CreateItems()
		{
			try
			{
		...
		/// PS_PP662_SetComboBox
		/// </summary>
		private void PS_PP662_SetComboBox()
		{
			string sQry;
		...
		/// PS_PP662_Initialize
		/// </summary>
		private void PS_PP662_Initialize()
		{
		...
		/// <summary>
		/// PS_PP662_PrintReport
		/// </summary>
		[STAThread]
		private void PS_PP662_PrintReport()
		{
			string WinTitle;
			string ReportName;
			string BPLId;
			...
			try
			{
				...
				FailCode = oForm.Items.Item("FailCode").Specific.Value.ToString().Trim();
				...
				{
					WinTitle = "[PS_PP662_01] 원인별 불량현황 (멀티)";
					ReportName = "PS_PP662_01.RPT";
				}
				else
				{
					WinTitle = "[PS_PP663_01] 원인별 불량현황(세부내역)-(멀티)";
					ReportName = "PS_PP663_01.RPT";
				}
				...
				dataPackParameter.Add(new PSH_DataPackClass("@FailCode", FailCode));
		...
		private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					{
						System.Threading.Thread thread = new System.Threading.Thread(PS_PP662_PrintReport);
		...
		private void Raise_EVENT_KEY_DOWN(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					dataHelpClass.ActiveUserDefineValue(ref oForm, ref pVal, ref BubbleEvent, "FailCode", "");
				}
				else if (pVal.BeforeAction == false)
				{
				}
			...
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							sQry = "SELECT U_SmalName FROM [@PS_PP003L] WHERE U_SmalCode = '" + oForm.Items.Item("FailCode").Specific.Value.ToString().Trim() + "'";
							oRecordSet.DoQuery(sQry);
							oForm.Items.Item("CodeName").Specific.String = oRecordSet.Fields.Item("U_SmalName").Value.ToString().Trim();
						}
					}
				}
			}
