...
namespace PSH_BOne_AddOn
{
    ...
    internal class PS_PP918 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFormDocEntry)
        {
            ...
            try
            {
                oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP918.srf");
                ...
                oFormUniqueID = "PS_PP918_" + SubMain.Get_TotalFormsCount();
                SubMain.Add_Forms(this, oFormUniqueID, "PS_PP918");
                ...
                oForm.Freeze(true);
                PS_PP918_CreateItems();
                PS_PP918_ComboBox_Setting();
        ...
        /// 화면 Item 생성
        /// </summary>
        private void PS_PP918_CreateItems()
        {
            try
            {
                //디비데이터 소스 개체 할당
        ...
        /// Initialization
        /// </summary>
        private void PS_PP918_Initialization()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
        ...
        /// </summary>
        /// <returns></returns>
        private bool PS_PP918_HeaderSpaceLineDel()
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(oDS_PS_PP918H.GetValue("U_ItemCode", 0)))
                {
                    errMessage = "제품코드는 필수입력사항입니다. 확인하세요.";
                }
                else if (string.IsNullOrEmpty(oDS_PS_PP918H.GetValue("U_BPLId", 0)))
                {
        ...
        /// 모드에 따른 아이템 설정
        /// </summary>
        private void PS_PP918_FormItemEnabled()
        {
            try
            {
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                {
                    ...
                    oForm.Items.Item("Size").Enabled = false;
                    oForm.Items.Item("Mark").Enabled = false;
                    oForm.Items.Item("ItemType").Enabled = false;
                }
                else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    oForm.Items.Item("ItemName").Enabled = true;
                    oForm.Items.Item("Size").Enabled = true;
                    oForm.Items.Item("Mark").Enabled = true;
                    oForm.Items.Item("ItemType").Enabled = true;
                }
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                oGrid01.DataTable = oForm.DataSources.DataTables.Item("PS_USERDS01");
            }
            catch (Exception ex)
            {
                if (errMessage != string.Empty)
            ...
            try
            {
                if (pVal.BeforeAction == true)
                {
                    if (pVal.ItemUID == "1")
                    {
                        ...
                        {
                            if (PS_PP918_HeaderSpaceLineDel() == false)
                            {
                                BubbleEvent = false;
                                return;
                            }
                            sQry = "Select IsNull(Max(Code), 0) + 1 From [@PS_PP918H]";
                            oRecordSet01.DoQuery(sQry);
                            Code = oRecordSet01.Fields.Item(0).Value.PadLeft(8,"0");
                            oDS_PS_PP918H.SetValue("Code", 0, Code);
                        }
                    }
                }
