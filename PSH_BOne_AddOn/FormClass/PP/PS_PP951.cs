...
namespace PSH_BOne_AddOn
{
	...
	/// 부문별 P/O대비 입고현황
	/// </summary>
	internal class PS_PP951 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_PP951.srf");
				...
				oFormUniqueID = "PS_PP951_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_PP951");
				...
				PS_PP951_CreateItems();
				PS_PP951_SetComboBox();
				PS_PP951_Initialize();
		...
		/// PS_PP951_CreateItems
		/// </summary>
		private void PS_PP951_CreateItems()
		{
			try
			{
		...
		/// PS_PP951_SetComboBox
		/// </summary>
		private void PS_PP951_SetComboBox()
		{
			string sQry;
		...
		/// PS_PP951_Initialize
		/// </summary>
		private void PS_PP951_Initialize()
		{
		...
		/// PS_PP951_LoadData
		/// </summary>
		private void PS_PP951_LoadData()
		{
			string YYYYMM;
			string BPLId;
			...
			try
			{
                ...
                {
					errMessage = "조회시 년월은 필수입력사항입니다. 확인하세요.";
					throw new Exception();
				}
		...
		private void Raise_EVENT_DOUBLE_CLICK(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					...
					{
						for (int i = 0; i <= oGrid01.Rows.Count - 1; i++)
						{
							if (oGrid01.Rows.IsSelected(i) == true)
							{
								...
								BPLId    = oForm.Items.Item("BPLId").Specific.Value.ToString().Trim();
								YYYYMM   = oForm.Items.Item("YYYYMM").Specific.Value.ToString().Trim();
								ItmBsort = oGrid01.DataTable.GetValue(0, i).ToString().Trim();
								...
								oForm.DataSources.DataTables.Item(1).ExecuteQuery(sQry);
								oGrid02.DataTable = oForm.DataSources.DataTables.Item("PS_USERDS02");
				...
				else if (pVal.BeforeAction == false)
				{
					if (pVal.ItemChanged == true)
					{
						if (pVal.ItemUID == "ItemCode")
						{
							...
							oForm.Items.Item("Size").Specific.Value = oRecordSet.Fields.Item("U_Size").Value.ToString().Trim();
							oForm.Items.Item("Mark").Specific.Select(oRecordSet.Fields.Item("U_Mark").Value.ToString().Trim());
							oForm.Items.Item("ItemType").Specific.Select(oRecordSet.Fields.Item("U_ItemType").Value.ToString().Trim());
						}
					}
				}
			}
