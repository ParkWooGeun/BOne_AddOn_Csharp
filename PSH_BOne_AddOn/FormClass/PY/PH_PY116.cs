    ...
    internal class PH_PY116 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFormDocEntry01)
        {
            ...
            try
            {
                ...
                PSH_Globals.ExecuteEventFilter(typeof(PH_PY116));
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private bool PH_PY116_DataValidCheck()
        {
        ...
        /// 매트릭스 행 추가
        /// </summary>
        private void PH_PY116_AddMatrixRow()
        {
            int oRow;
            ...
            try
            {
                ...
                for (k = 0; k <= oMat1.VisualRowCount - 1; k++)
                {
                    ...
                    if (!string.IsNullOrEmpty(oDS_PH_PY116B.GetValue("U_Col02", k).ToString().Trim()))
                    {
                        ...
                        if (oDIM3 == "Y")
                        {
                            _with1.JournalEntries.Lines.CostingCode3 = Dimenz3.ToString().Trim(); //차원3
                        }
        ...
        private void TOTAL_AMT()
        {
            ...
            try
            {
                ...
                {
                    oDS_PH_PY116B.Offset = oRow - 1;
                    oTOTDEB += Convert.ToDouble(oDS_PH_PY116B.GetValue("U_Debit", oRow - 1).Replace(",", ""));
                    oTOTCRE += Convert.ToDouble(oDS_PH_PY116B.GetValue("U_Credit", oRow - 1).Replace(",", ""));
                }
                oForm.Items.Item("TOTDEB").Specific.Value = oTOTDEB;
                oForm.Items.Item("TOTCRE").Specific.Value = oTOTCRE;
            }
