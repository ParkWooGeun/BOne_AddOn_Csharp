    ...
    internal class PH_PY135 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFormDocEntry01)
        {
            ...
            try
            {
                ...
                PSH_Globals.ExecuteEventFilter(typeof(PH_PY135));
            }
            catch (Exception ex)
            {
        ...
        private void PH_PY135_FlushToItemValue(string oUID, int oRow, string oCol)
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
        ...
        private void PH_PY135_MTX01()
        {
            ...
            try
            {
                ...
                for (i = 0; i <= oRecordSet01.RecordCount - 1; i++)
                {
                    ...
                    totalDebit += Convert.ToDouble(oRecordSet01.Fields.Item("Debit").Value);
                    totalCredit += Convert.ToDouble(oRecordSet01.Fields.Item("Credit").Value);
        ...
        private bool PH_PY135_Create_oJournalEntries(int ChkType)
        {
            ...
            string sShortName; //GL계정
            string sAcctCode; //관리계정
            string sDocDate;
            string sPrcCode;
            string sPrcName;
            string BPLid;
            ...
            try
            {
                ...
                if (ChkType == 1)
                {
                    ...
                    sQry += "         U_JdtCC = '" + sCC + "'";
                    sQry += " WHERE   DocNum = '" + oDS_PH_PY135A.GetValue("DocNum", 0).ToString().Trim() + "'";
                ...
                if (pVal.BeforeAction == true)
                {
                    ...
                    else if (pVal.ItemUID == "Btn02") //분개확정 버튼
                    {
                        if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                        {
                            ...
                            {
                                PH_PY135_Item_Error_Message(1);
                                BubbleEvent = false;
                                return;
                            }
                            else if (oForm.Items.Item("Status").Specific.Value == "C")
                            {
                                PH_PY135_Item_Error_Message(2);
                                BubbleEvent = false;
                                return;
                            }
                            else
        ...
        private void Raise_EVENT_VALIDATE(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                if (pVal.BeforeAction == true)
                {
                    ...
                    {
                        PH_PY135_FlushToItemValue(pVal.ItemUID, 0, "");
                    }
                }
                else if (pVal.BeforeAction == false)
                {
