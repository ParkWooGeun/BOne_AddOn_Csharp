...
namespace PSH_BOne_AddOn
{
    ...
    /// 근속보전휴가발생및사용내역
    /// </summary>
    internal class PH_PY605 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFromDocEntry01)
        {
            ...
            try
            {
                oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PH_PY605.srf");
                ...
                oFormUniqueID01 = "PH_PY605_" + SubMain.Get_TotalFormsCount();
                SubMain.Add_Forms(this, oFormUniqueID01, "PH_PY605");
                ...
                oForm.Freeze(true);
                PH_PY605_CreateItems();
                PH_PY605_FormItemEnabled();
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private void PH_PY605_CreateItems()
        {
            ...
            try
            {
                ...
                sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = '9' AND U_UseYN= 'Y' AND U_Char1 = '" + oForm.Items.Item("RspCode").Specific.VALUE.ToString().Trim() + "'";
                ...
                ////시작직원구분
                oForm.DataSources.UserDataSources.Add("JIGTYPFr", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
                oForm.Items.Item("JIGTYPFr").Specific.DataBind.SetBound(true, "", "JIGTYPFr");
                sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P126' AND U_UseYN= 'Y'";
                dataHelpClass.SetReDataCombo(oForm, sQry, oForm.Items.Item("JIGTYPFr").Specific, "Y");
                oForm.Items.Item("JIGTYPFr").DisplayDesc = true;

                ////종료직원구분
                oForm.DataSources.UserDataSources.Add("JIGTYPTo", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
                oForm.Items.Item("JIGTYPTo").Specific.DataBind.SetBound(true, "", "JIGTYPTo");
                sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P126' AND U_UseYN= 'Y'";
                dataHelpClass.SetReDataCombo(oForm, sQry, oForm.Items.Item("JIGTYPTo").Specific, "Y");
                oForm.Items.Item("JIGTYPTo").DisplayDesc = true;
            }
            catch (Exception ex)
            {
                PSH_Globals.SBO_Application.SetStatusBarMessage("PH_PY605_CreateItems_Error:" + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, true);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        ///// 화면의 아이템 Enable 설정
        ///// </summary>
        public void PH_PY605_FormItemEnabled()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            try
            {
            ...
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY605_FormItemEnabled_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                if (pVal.BeforeAction == true)
                {
                    ...
                    {
                        System.Threading.Thread thread = new System.Threading.Thread(PH_PY605_Print_Report01);
        ...
        /// </summary>
        [STAThread]
        private void PH_PY605_Print_Report01()
        {
            ...
            string JIGTYPFr = string.Empty;
            string JIGTYPTo = string.Empty;
            ...
            try
            {
                WinTitle = "[PH_PY605] 근속보전휴가발생및사용내역";
                ReportName = "PH_PY605_01.rpt";
                ...
                dataPackParameter.Add(new PSH_DataPackClass("@JIGTYPFr", JIGTYPFr)); //
                dataPackParameter.Add(new PSH_DataPackClass("@JIGTYPTo", JIGTYPTo)); //
                dataPackParameter.Add(new PSH_DataPackClass("@ClsCode", ClsCode)); //

                //SubReport Parameter
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@CLTCOD", CLTCOD, "PH_PY605_SUB1"));
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@YY", YY, "PH_PY605_SUB1"));
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@Sabun", MSTCOD, "PH_PY605_SUB1"));
            ...
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY605_Print_Report01_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
            }
...
////Public oDS_PH_PY605     As SAPbouiCOM.DataTable
////
////'// 매트릭스 사용시
////Public oMat1 As SAPbouiCOM.Matrix
////Private oDS_PH_PY605A As SAPbouiCOM.DBDataSource
////Private oDS_PH_PY605B As SAPbouiCOM.DBDataSource
...
//			oXmlDoc.load(MDC_Globals.SP_Path + "\\" + MDC_Globals.SP_Screen + "\\PH_PY605.srf");
...
//				oXmlDoc.selectNodes("Application/forms/action/form/items/action/item/specific/@cellHeight")[i - 1].nodeValue = 16;
//			}
//			oFormUniqueID = "PH_PY605_" + GetTotalFormsCount();
//			SubMain.AddForms(this, oFormUniqueID, "PH_PY605");
...
//			oForm.Freeze(true);
//			PH_PY605_CreateItems();
//			PH_PY605_EnableMenus();
//			PH_PY605_SetDocument(oFromDocEntry01);
//			//    Call PH_PY605_FormResize
...
//		private bool PH_PY605_CreateItems()
//		{
...
//			////시작직원구분
//			oForm.DataSources.UserDataSources.Add("JIGTYPFr", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
//			oCombo = oForm.Items.Item("JIGTYPFr").Specific;
//			oCombo.DataBind.SetBound(true, "", "JIGTYPFr");
//			sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P126' AND U_UseYN= 'Y'";
//			MDC_SetMod.SetReDataCombo(oForm, sQry, ref oCombo, ref "Y");
//			oForm.Items.Item("JIGTYPFr").DisplayDesc = true;
...
//			////종료직원구분
//			oForm.DataSources.UserDataSources.Add("JIGTYPTo", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
//			oCombo = oForm.Items.Item("JIGTYPTo").Specific;
//			oCombo.DataBind.SetBound(true, "", "JIGTYPTo");
//			sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P126' AND U_UseYN= 'Y'";
//			MDC_SetMod.SetReDataCombo(oForm, sQry, ref oCombo, ref "Y");
//			oForm.Items.Item("JIGTYPTo").DisplayDesc = true;
...
//			oRecordSet = null;
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_CreateItems_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}

//		private void PH_PY605_EnableMenus()
//		{
...
//			return;
//			PH_PY605_EnableMenus_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_EnableMenus_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		private void PH_PY605_SetDocument(string oFromDocEntry01)
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			}
//			return;
//			PH_PY605_SetDocument_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_SetDocument_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//			oForm.Freeze(false);
//			return;
//			PH_PY605_FormItemEnabled_Error:

//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_FormItemEnabled_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//						//UPGRADE_NOTE: oForm 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//						oForm = null;
//						//                Set oDS_PH_PY605A = Nothing
//						//                Set oDS_PH_PY605B = Nothing
...
//						//
//						//                    While (i <= oDS_PH_PY605B.Size - 1)
//						//                        If oDS_PH_PY605B.GetValue("U_FILD01", i) = "" Then
//						//                            oDS_PH_PY605B.RemoveRecord (i)
//						//                            i = 0
//						//                        Else
//						//                            i = i + 1
//						//                        End If
//						//                    Wend
//						//
//						//                    For i = 0 To oDS_PH_PY605B.Size
//						//                        Call oDS_PH_PY605B.setValue("U_LineNum", i, i + 1)
//						//                    Next i
//						//
//						//                    oMat1.LoadFromDataSource
//						//End If
...
//		public void PH_PY605_AddMatrixRow()
//		{
//			int oRow = 0;
...
//			//        If Trim(oDS_PH_PY605B.GetValue("U_FILD01", oRow - 1)) <> "" Then
//			//            If oDS_PH_PY605B.Size <= oMat1.VisualRowCount Then
//			//                oDS_PH_PY605B.InsertRecord (oRow)
//			//            End If
//			//            oDS_PH_PY605B.Offset = oRow
//			//            oDS_PH_PY605B.setValue "U_LineNum", oRow, oRow + 1
//			//            oDS_PH_PY605B.setValue "U_FILD01", oRow, ""
//			//            oDS_PH_PY605B.setValue "U_FILD02", oRow, ""
//			//            oDS_PH_PY605B.setValue "U_FILD03", oRow, 0
//			//            oMat1.LoadFromDataSource
//			//        Else
//			//            oDS_PH_PY605B.Offset = oRow - 1
//			//            oDS_PH_PY605B.setValue "U_LineNum", oRow - 1, oRow
//			//            oDS_PH_PY605B.setValue "U_FILD01", oRow - 1, ""
//			//            oDS_PH_PY605B.setValue "U_FILD02", oRow - 1, ""
//			//            oDS_PH_PY605B.setValue "U_FILD03", oRow - 1, 0
//			//            oMat1.LoadFromDataSource
//			//        End If
...
//			oForm.Freeze(false);
//			return;
//			PH_PY605_AddMatrixRow_Error:
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_AddMatrixRow_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public void PH_PY605_FormClear()
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement

//			string DocEntry = null;
//			//UPGRADE_WARNING: MDC_GetData.Get_ReData() 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			DocEntry = MDC_GetData.Get_ReData(ref "AutoKey", ref "ObjectCode", ref "ONNM", ref "'PH_PY605'", ref "");
...
//			}
//			return;
//			PH_PY605_FormClear_Error:
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_FormClear_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public bool PH_PY605_DataValidCheck()
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			//    '//필수 체크
//			//    '//----------------------------------------------------------------------------------
//			//    If Trim(oDS_PH_PY605A.GetValue("Code", 0)) = "" Then
//			//        Sbo_Application.SetStatusBarMessage "사원번호는 필수입니다.", bmt_Short, True
//			//        oForm.Items("Code").CLICK ct_Regular
//			//        PH_PY605_DataValidCheck = False
//			//        Exit Function
//			//    End If
//			//
//			//    oMat1.FlushToDataSource
//			//    '// Matrix 마지막 행 삭제(DB 저장시)
//			//    If oDS_PH_PY605B.Size > 1 Then oDS_PH_PY605B.RemoveRecord (oDS_PH_PY605B.Size - 1)
//			//    oMat1.LoadFromDataSource
...
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			PH_PY605_DataValidCheck_Error:


//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_DataValidCheck_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}


//		public bool PH_PY605_Validate(string ValidateType)
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			//UPGRADE_WARNING: MDC_Company_Common.GetValue(SELECT Canceled FROM [PH_PY605A] WHERE DocEntry = ' & oForm.Items(DocEntry).Specific.VALUE & ', 0, 1) 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			if (MDC_Company_Common.GetValue("SELECT Canceled FROM [@PH_PY605A] WHERE DocEntry = '" + oForm.Items.Item("DocEntry").Specific.VALUE + "'", 0, 1) == "Y") {
...
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY605_Validate_Exit:
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY605_Validate_Error:
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_Validate_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}
...
//			string RspCode = null;
//			string MSTCOD = null;
//			string JIGTYPFr = null;
//			string JIGTYPTo = null;
//			string ClsCode = null;
//			//반코드(2013.05.13 송명규 추가)
...
//				goto PH_PY605_Print_Report01_Error;
//			}
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			JIGTYPFr = Strings.Trim(oForm.Items.Item("JIGTYPFr").Specific.VALUE);
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			JIGTYPTo = Strings.Trim(oForm.Items.Item("JIGTYPTo").Specific.VALUE);
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			ClsCode = Strings.Trim(oForm.Items.Item("ClsCode").Specific.VALUE);

//			if (string.IsNullOrEmpty(JIGTYPFr))
//				JIGTYPFr = "00";
//			if (string.IsNullOrEmpty(JIGTYPTo))
//				JIGTYPTo = "99";
...
//			sQry = "EXEC [PH_PY605_02] '" + CLTCOD + "', '" + YY + "', '" + MSTCOD + "'";
//			MDC_Globals.gRpt_SRptSqry[1] = sQry;
//			MDC_Globals.gRpt_SRptName[1] = "PH_PY605_SUB1";


//			/// Procedure 실행"
//			sQry = "EXEC [PH_PY605_01] '" + CLTCOD + "', '" + YY + "', '" + TeamCode + "', '" + RspCode + "', '" + MSTCOD + "', '" + JIGTYPFr + "', '" + JIGTYPTo + "','" + ClsCode + "'";
...
//			} else {
//				MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY605_Print_Report01_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			}
