...
namespace PSH_BOne_AddOn
{
    ...
    /// 근태구분별사용현황
    /// </summary>
    internal class PH_PY610 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFromDocEntry01)
        {
            ...
            try
            {
                oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PH_PY610.srf");
                ...
                oFormUniqueID01 = "PH_PY610_" + SubMain.Get_TotalFormsCount();
                SubMain.Add_Forms(this, oFormUniqueID01, "PH_PY610");
                ...
                oForm.Freeze(true);
                PH_PY610_CreateItems();
                PH_PY610_FormItemEnabled();
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private void PH_PY610_CreateItems()
        {
            ...
            {
                PSH_Globals.SBO_Application.SetStatusBarMessage("PH_PY610_CreateItems_Error:" + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, true);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        ///// 화면의 아이템 Enable 설정
        ///// </summary>
        public void PH_PY610_FormItemEnabled()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            try
            {
            ...
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY610_FormItemEnabled_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                if (pVal.BeforeAction == true)
                {
                    ...
                    {
                        System.Threading.Thread thread = new System.Threading.Thread(PH_PY610_Print_Report01);
        ...
        /// </summary>
        [STAThread]
        private void PH_PY610_Print_Report01()
        {
            ...
            WorkType =  oForm.Items.Item("WorkType").Specific.VALUE.Trim();
            ...
            try
            {
                WinTitle = "[PH_PY610] 근태구분별사용현황";
                ReportName = "PH_PY610_01.rpt";
                ...
                dataPackFormula.Add(new PSH_DataPackClass("@WorkName", dataHelpClass.Get_ReData("U_CodeNm", "U_Code", "[@PS_HR200L]", "'" + WorkType + "'", "and Code = 'P221' AND U_UseYN= 'Y'"))); //
                ...
                dataPackParameter.Add(new PSH_DataPackClass("@WorkType", WorkType)); //

                //SubReport Parameter
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@CLTCOD", CLTCOD, "PH_PY610_SUB1"));
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@DocDateFr", DocDateFr, "PH_PY610_SUB1"));
                dataPackSubReportParameter.Add(new PSH_DataPackClass("@DocDateTo", DocDateTo, "PH_PY610_SUB1"));
            ...
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY610_Print_Report01_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
            }
...
////Public oDS_PH_PY610     As SAPbouiCOM.DataTable
////
////'// 매트릭스 사용시
////Public oMat1 As SAPbouiCOM.Matrix
////Private oDS_PH_PY610A As SAPbouiCOM.DBDataSource
////Private oDS_PH_PY610B As SAPbouiCOM.DBDataSource
...
//			oXmlDoc.load(MDC_Globals.SP_Path + "\\" + MDC_Globals.SP_Screen + "\\PH_PY610.srf");
...
//				oXmlDoc.selectNodes("Application/forms/action/form/items/action/item/specific/@cellHeight")[i - 1].nodeValue = 16;
//			}
//			oFormUniqueID = "PH_PY610_" + GetTotalFormsCount();
//			SubMain.AddForms(this, oFormUniqueID, "PH_PY610");
...
//			oForm.Freeze(true);
//			PH_PY610_CreateItems();
//			PH_PY610_EnableMenus();
//			PH_PY610_SetDocument(oFromDocEntry01);
//			//    Call PH_PY610_FormResize
...
//		private bool PH_PY610_CreateItems()
//		{
...
//			////근태코드
//			oForm.DataSources.UserDataSources.Add("WorkType", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
//			oCombo = oForm.Items.Item("WorkType").Specific;
//			oCombo.DataBind.SetBound(true, "", "WorkType");
//			sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P221' AND U_UseYN= 'Y'";
...
//			oForm.Items.Item("DocDateFr").Specific.DataBind.SetBound(true, "", "DocDateFr");
//			oForm.DataSources.UserDataSources.Item("DocDateFr").Value = Microsoft.VisualBasic.Compatibility.VB6.Support.Format(DateAndTime.Now, "YYYYMMDD");
...
//			oRecordSet = null;
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_CreateItems_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}

//		private void PH_PY610_EnableMenus()
//		{
...
//			return;
//			PH_PY610_EnableMenus_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_EnableMenus_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		private void PH_PY610_SetDocument(string oFromDocEntry01)
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			}
//			return;
//			PH_PY610_SetDocument_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_SetDocument_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//			oForm.Freeze(false);
//			return;
//			PH_PY610_FormItemEnabled_Error:

//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_FormItemEnabled_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//					if (pval.BeforeAction == true) {
//						if (pval.ItemUID == "Btn01") {
//							PH_PY610_DataValidCheck();
//							PH_PY610_Print_Report01();
//						}
//					} else if (pval.BeforeAction == false) {
...
//									////현재 사업장으로 다시 Qry
//									//UPGRADE_WARNING: oForm.Items.Item().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//									sQry = "SELECT U_Code, U_CodeNm FROM [@PS_HR200L] WHERE Code = '2' AND U_UseYN= 'Y' AND U_Char1 = '" + oForm.Items.Item("TeamCode").Specific.VALUE + "' AND U_Char2 = '" +  oForm.Items.Item("CLTCOD").Specific.VALUE) + "'";
//									MDC_SetMod.SetReDataCombo(oForm, sQry, ref oCombo, ref "Y");
//									break;
...
//						//UPGRADE_NOTE: oForm 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//						oForm = null;
//						//                Set oDS_PH_PY610A = Nothing
//						//                Set oDS_PH_PY610B = Nothing
...
//						//
//						//                    While (i <= oDS_PH_PY610B.Size - 1)
//						//                        If oDS_PH_PY610B.GetValue("U_FILD01", i) = "" Then
//						//                            oDS_PH_PY610B.RemoveRecord (i)
//						//                            i = 0
//						//                        Else
//						//                            i = i + 1
//						//                        End If
//						//                    Wend
//						//
//						//                    For i = 0 To oDS_PH_PY610B.Size
//						//                        Call oDS_PH_PY610B.setValue("U_LineNum", i, i + 1)
//						//                    Next i
//						//
//						//                    oMat1.LoadFromDataSource
//						//End If
...
//		public void PH_PY610_AddMatrixRow()
//		{
//			int oRow = 0;
...
//			//        If Trim(oDS_PH_PY610B.GetValue("U_FILD01", oRow - 1)) <> "" Then
//			//            If oDS_PH_PY610B.Size <= oMat1.VisualRowCount Then
//			//                oDS_PH_PY610B.InsertRecord (oRow)
//			//            End If
//			//            oDS_PH_PY610B.Offset = oRow
//			//            oDS_PH_PY610B.setValue "U_LineNum", oRow, oRow + 1
//			//            oDS_PH_PY610B.setValue "U_FILD01", oRow, ""
//			//            oDS_PH_PY610B.setValue "U_FILD02", oRow, ""
//			//            oDS_PH_PY610B.setValue "U_FILD03", oRow, 0
//			//            oMat1.LoadFromDataSource
//			//        Else
//			//            oDS_PH_PY610B.Offset = oRow - 1
//			//            oDS_PH_PY610B.setValue "U_LineNum", oRow - 1, oRow
//			//            oDS_PH_PY610B.setValue "U_FILD01", oRow - 1, ""
//			//            oDS_PH_PY610B.setValue "U_FILD02", oRow - 1, ""
//			//            oDS_PH_PY610B.setValue "U_FILD03", oRow - 1, 0
//			//            oMat1.LoadFromDataSource
//			//        End If
...
//			oForm.Freeze(false);
//			return;
//			PH_PY610_AddMatrixRow_Error:
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_AddMatrixRow_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public void PH_PY610_FormClear()
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement

//			string DocEntry = null;
//			//UPGRADE_WARNING: MDC_GetData.Get_ReData() 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			DocEntry = MDC_GetData.Get_ReData(ref "AutoKey", ref "ObjectCode", ref "ONNM", ref "'PH_PY610'", ref "");
...
//			}
//			return;
//			PH_PY610_FormClear_Error:
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_FormClear_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public bool PH_PY610_DataValidCheck()
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			////----------------------------------------------------------------------------------
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			if (string.IsNullOrEmpty( oForm.Items.Item("WorkType").Specific.VALUE))) {
//				MDC_Globals.Sbo_Application.SetStatusBarMessage("근태코드는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
...
//			//    If oDS_PH_PY610B.Size > 1 Then oDS_PH_PY610B.RemoveRecord (oDS_PH_PY610B.Size - 1)
//			//    oMat1.LoadFromDataSource
...
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			PH_PY610_DataValidCheck_Error:


//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_DataValidCheck_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}


//		public bool PH_PY610_Validate(string ValidateType)
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			//UPGRADE_WARNING: MDC_Company_Common.GetValue(SELECT Canceled FROM [PH_PY610A] WHERE DocEntry = ' & oForm.Items(DocEntry).Specific.VALUE & ', 0, 1) 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			if (MDC_Company_Common.GetValue("SELECT Canceled FROM [@PH_PY610A] WHERE DocEntry = '" + oForm.Items.Item("DocEntry").Specific.VALUE + "'", 0, 1) == "Y") {
...
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY610_Validate_Exit:
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY610_Validate_Error:
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_Validate_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}
...
//				goto PH_PY610_Print_Report01_Error;
//			}
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			WorkType =  oForm.Items.Item("WorkType").Specific.VALUE);
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
...
//			MDC_Globals.gRpt_Formula[4] = "WorkName";
//			sQry = "SELECT U_CodeNm FROM [@PS_HR200L] WHERE Code = 'P221' AND U_UseYN= 'Y' AND U_Code = '" + WorkType + "'";
//			oRecordSet.DoQuery(sQry);
//			//UPGRADE_WARNING: oRecordSet.Fields().VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			MDC_Globals.gRpt_Formula_Value[4] = oRecordSet.Fields.Item(0).Value;
...
//			sQry = "EXEC [PH_PY610_02] '" + CLTCOD + "', '" + DocDateFr + "',  '" + DocDateTo + "', '" + MSTCOD + "', '" + WorkType + "'";
//			MDC_Globals.gRpt_SRptSqry[1] = sQry;
//			MDC_Globals.gRpt_SRptName[1] = "PH_PY610_SUB1";


//			/// Procedure 실행"
//			sQry = "EXEC [PH_PY610_01] '" + CLTCOD + "', '" + DocDateFr + "',  '" + DocDateTo + "', '" + TeamCode + "', '" + RspCode + "', '" + MSTCOD + "', '" + WorkType + "'";
...
//				MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY610_Print_Report01_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
