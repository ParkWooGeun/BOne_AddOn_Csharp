...
namespace PSH_BOne_AddOn
{
    ...
    /// 상여부서별집계대장
    /// </summary>
    internal class PH_PY720 : PSH_BaseClass
    {
        ...
        public override void LoadForm(string oFromDocEntry01)
        {
            ...
            try
            {
                oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PH_PY720.srf");
                ...
                oFormUniqueID01 = "PH_PY720_" + SubMain.Get_TotalFormsCount();
                SubMain.Add_Forms(this, oFormUniqueID01, "PH_PY720");
                ...
                oForm.Freeze(true);
                PH_PY720_CreateItems();
                PH_PY720_FormItemEnabled();
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private void PH_PY720_CreateItems()
        {
            ...
            try
            {
                ...
                oForm.Items.Item("GUBUN").Specific.ValidValues.Add("1", "일반");
                oForm.Items.Item("GUBUN").Specific.ValidValues.Add("2", "보고용");
            ...
            {
                PSH_Globals.SBO_Application.SetStatusBarMessage("PH_PY720_CreateItems_Error:" + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, true);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        /// 화면의 아이템 Enable 설정
        /// </summary>
        public void PH_PY720_FormItemEnabled()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            try
            {
            ...
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY720_FormItemEnabled_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
                oForm.Freeze(false);
        ...
        private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                if (pVal.BeforeAction == true)
                {
                    ...
                    {
                        if (PH_PY720_DataValidCheck() == false)    // 필수 아이템Check후 실행
                        {
                            System.Threading.Thread thread = new System.Threading.Thread(PH_PY720_Print_Report01);
        ...
        /// <summary>
        /// PH_PY720_DataValidCheck
        /// </summary>
        /// <returns></returns>
        private bool PH_PY720_DataValidCheck()
        {
            ...
            {
                PSH_Globals.SBO_Application.SetStatusBarMessage("PH_PY720_DataValidCheck_Error:" + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, true);
            }
            finally
            {
        ...
        /// </summary>
        [STAThread]
        private void PH_PY720_Print_Report01()
        {
            ...
            try
            {
                WinTitle = "[PH_PY720] 상여부서별집계대장";

                if (OptBtnValue == "1")
                {
                    ReportName = "PH_PY720_01.rpt";  //담당별
                }
                else
                {
                    ReportName = "PH_PY720_02.rpt";   //반별
                }
                ...
                dataPackParameter.Add(new PSH_DataPackClass("@GUBUN", GUBUN)); //집계구분 

                formHelpClass.CrystalReportOpen(WinTitle, ReportName, dataPackParameter, dataPackFormula);
            }
            catch (Exception ex)
            {
                PSH_Globals.SBO_Application.StatusBar.SetText("PH_PY720_Print_Report01_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
            }
            finally
            {
            }
...
////Public oDS_PH_PY720     As SAPbouiCOM.DataTable
////
////'// 매트릭스 사용시
////Public oMat1 As SAPbouiCOM.Matrix
////Private oDS_PH_PY720A As SAPbouiCOM.DBDataSource
////Private oDS_PH_PY720B As SAPbouiCOM.DBDataSource
...
//			oXmlDoc.load(MDC_Globals.SP_Path + "\\" + MDC_Globals.SP_Screen + "\\PH_PY720.srf");
...
//				oXmlDoc.selectNodes("Application/forms/action/form/items/action/item/specific/@cellHeight")[i - 1].nodeValue = 16;
//			}
//			oFormUniqueID = "PH_PY720_" + GetTotalFormsCount();
//			SubMain.AddForms(this, oFormUniqueID, "PH_PY720");
...
//			oForm.Freeze(true);
//			PH_PY720_CreateItems();
//			PH_PY720_EnableMenus();
//			PH_PY720_SetDocument(oFromDocEntry01);
//			//    Call PH_PY720_FormResize
...
//		private bool PH_PY720_CreateItems()
//		{
//			bool functionReturnValue = false;
...
//			oCombo.ValidValues.Add("1", "일반");
//			oCombo.ValidValues.Add("2", "보고용");
...
//			oRecordSet = null;
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_CreateItems_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}

//		private void PH_PY720_EnableMenus()
//		{
...
//			return;
//			PH_PY720_EnableMenus_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_EnableMenus_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		private void PH_PY720_SetDocument(string oFromDocEntry01)
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			}
//			return;
//			PH_PY720_SetDocument_Error:

//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_SetDocument_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//			oForm.Freeze(false);
//			return;
//			PH_PY720_FormItemEnabled_Error:

//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_FormItemEnabled_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}
...
//					if (pval.BeforeAction == true) {
//						if (pval.ItemUID == "Btn01") {
//							if (PH_PY720_DataValidCheck() == true) {
//								PH_PY720_Print_Report01();
//							}
...
//						//UPGRADE_NOTE: oForm 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//						oForm = null;
//						//                Set oDS_PH_PY720A = Nothing
//						//                Set oDS_PH_PY720B = Nothing
...
//						//
//						//                    While (i <= oDS_PH_PY720B.Size - 1)
//						//                        If oDS_PH_PY720B.GetValue("U_FILD01", i) = "" Then
//						//                            oDS_PH_PY720B.RemoveRecord (i)
//						//                            i = 0
//						//                        Else
//						//                            i = i + 1
//						//                        End If
//						//                    Wend
//						//
//						//                    For i = 0 To oDS_PH_PY720B.Size
//						//                        Call oDS_PH_PY720B.setValue("U_LineNum", i, i + 1)
//						//                    Next i
//						//
//						//                    oMat1.LoadFromDataSource
//						//End If
...
//		public void PH_PY720_AddMatrixRow()
//		{
//			int oRow = 0;
...
//			//        If Trim(oDS_PH_PY720B.GetValue("U_FILD01", oRow - 1)) <> "" Then
//			//            If oDS_PH_PY720B.Size <= oMat1.VisualRowCount Then
//			//                oDS_PH_PY720B.InsertRecord (oRow)
//			//            End If
//			//            oDS_PH_PY720B.Offset = oRow
//			//            oDS_PH_PY720B.setValue "U_LineNum", oRow, oRow + 1
//			//            oDS_PH_PY720B.setValue "U_FILD01", oRow, ""
//			//            oDS_PH_PY720B.setValue "U_FILD02", oRow, ""
//			//            oDS_PH_PY720B.setValue "U_FILD03", oRow, 0
//			//            oMat1.LoadFromDataSource
//			//        Else
//			//            oDS_PH_PY720B.Offset = oRow - 1
//			//            oDS_PH_PY720B.setValue "U_LineNum", oRow - 1, oRow
//			//            oDS_PH_PY720B.setValue "U_FILD01", oRow - 1, ""
//			//            oDS_PH_PY720B.setValue "U_FILD02", oRow - 1, ""
//			//            oDS_PH_PY720B.setValue "U_FILD03", oRow - 1, 0
//			//            oMat1.LoadFromDataSource
//			//        End If
...
//			oForm.Freeze(false);
//			return;
//			PH_PY720_AddMatrixRow_Error:
//			oForm.Freeze(false);
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_AddMatrixRow_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public void PH_PY720_FormClear()
//		{
//			 // ERROR: Not supported in C#: OnErrorStatement

//			string DocEntry = null;
//			//UPGRADE_WARNING: MDC_GetData.Get_ReData() 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			DocEntry = MDC_GetData.Get_ReData(ref "AutoKey", ref "ObjectCode", ref "ONNM", ref "'PH_PY720'", ref "");
...
//			}
//			return;
//			PH_PY720_FormClear_Error:
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_FormClear_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//		}

//		public bool PH_PY720_DataValidCheck()
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//				MDC_Globals.Sbo_Application.SetStatusBarMessage("지급일자는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
...
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			PH_PY720_DataValidCheck_Error:


//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_DataValidCheck_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}


//		public bool PH_PY720_Validate(string ValidateType)
//		{
//			bool functionReturnValue = false;
//			 // ERROR: Not supported in C#: OnErrorStatement
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			//UPGRADE_WARNING: MDC_Company_Common.GetValue(SELECT Canceled FROM [PH_PY720A] WHERE DocEntry = ' & oForm.Items(DocEntry).Specific.VALUE & ', 0, 1) 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			if (MDC_Company_Common.GetValue("SELECT Canceled FROM [@PH_PY720A] WHERE DocEntry = '" + oForm.Items.Item("DocEntry").Specific.VALUE + "'", 0, 1) == "Y") {
...
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY720_Validate_Exit:
//			//UPGRADE_NOTE: oRecordSet 개체는 가비지가 수집되어야 소멸됩니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
//			oRecordSet = null;
//			return functionReturnValue;
//			PH_PY720_Validate_Error:
//			functionReturnValue = false;
//			MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_Validate_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			return functionReturnValue;
//		}
...
//				goto PH_PY720_Print_Report01_Error;
//			}
...
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
//			JIGBIL = Strings.Trim(oForm.Items.Item("JIGBIL").Specific.VALUE);
//			//UPGRADE_WARNING: oForm.Items().Specific.VALUE 개체의 기본 속성을 확인할 수 없습니다. 자세한 내용은 다음을 참조하십시오. 'ms-help://MS.VSExpressCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
...
//			WinTitle = "[PH_PY720] 상여부서별집계대장";
//			if (OptBtnValue == "1") {
//				ReportName = "PH_PY720_01.rpt";
//			} else {
//				ReportName = "PH_PY720_02.rpt";
//			}
...
//			MDC_Globals.gRpt_Formula_Value[2] = Microsoft.VisualBasic.Compatibility.VB6.Support.Format(YM, "####년 ##월");

//			MDC_Globals.gRpt_Formula[3] = "DocDate";
//			MDC_Globals.gRpt_Formula_Value[3] = Microsoft.VisualBasic.Compatibility.VB6.Support.Format(JIGBIL, "####-##-##");
...
//			/// Procedure 실행"
//			if (OptBtnValue == "1") {
//				sQry = "EXEC [PH_PY720_01] '" + CLTCOD + "', '" + JOBGBN + "',  '" + YM + "',  '" + JIGBIL + "', '" + PAYTYP + "', '" + Gubun + "'";
//			} else {
//				sQry = "EXEC [PH_PY720_02] '" + CLTCOD + "', '" + JOBGBN + "',  '" + YM + "',  '" + JIGBIL + "', '" + PAYTYP + "', '" + Gubun + "'";
//			}
...
//			} else {
//				MDC_Globals.Sbo_Application.SetStatusBarMessage("PH_PY720_Print_Report01_Error: " + Err().Number + " - " + Err().Description, SAPbouiCOM.BoMessageTime.bmt_Short, true);
//			}
