...
namespace PSH_BOne_AddOn
{
    ...
    internal class PS_QM703 : PSH_BaseClass
    {
        ...
        private SAPbouiCOM.DBDataSource oDS_PS_QM703H; //등록헤더
        private SAPbouiCOM.DBDataSource oDS_PS_QM703L; //등록라인
        ...
        public override void LoadForm(string oFromDocEntry01)
        {
            ...
            try
            {
                oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_QM703.srf");
                ...
                oFormUniqueID = "PS_QM703_" + SubMain.Get_TotalFormsCount();
                SubMain.Add_Forms(this, oFormUniqueID, "PS_QM703");
                ...
                oForm.Freeze(true);
                PS_QM703_CreateItems();
                PS_QM703_ComboBox_Setting();
                PS_QM703_EnableMenus();
                PS_QM703_SetDocument(oFromDocEntry01);
        ...
        /// 화면 Item 생성
        /// </summary>
        private void PS_QM703_CreateItems()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();

            try
            {
                oDS_PS_QM703H = oForm.DataSources.DBDataSources.Item("@PS_QM703H");
                oDS_PS_QM703L = oForm.DataSources.DBDataSources.Item("@PS_QM703L");
        ...
        /// Combobox 설정
        /// </summary>
        private void PS_QM703_ComboBox_Setting()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            string sQry;
            ...
            try
            {
                ...
                sQry += " '선택' AS [Name]";
                sQry += " UNION ALL";
                sQry += " SELECT U_Code AS [Code],U_CodeNm AS [Name] FROM [@PS_QM700L] WHERE Code ='ZCase' AND U_UseYN <>'N' ORDER BY Code";
                oRecordSet.DoQuery(sQry);
                while (!oRecordSet.EoF)
                {
                ...
                sQry += " '선택' AS [Name]";
                sQry += " UNION ALL";
                sQry += " SELECT U_Code AS [Code],U_CodeNm AS [Name] FROM [@PS_QM700L] WHERE Code ='ZOpinio' AND U_UseYN <>'N' ORDER BY Code";
                oRecordSet.DoQuery(sQry);
                while (!oRecordSet.EoF)
                {
                ...
                //메트릭스 콤보박스
                sQry = " SELECT U_Code AS [Code],U_CodeNm AS [Name] FROM [@PS_QM700L] WHERE Code ='ZGubun' AND U_UseYN <>'N' ORDER BY Code";
                oRecordSet.DoQuery(sQry);
                while (!oRecordSet.EoF)
                {
        ...
        private void PS_QM703_SetDocument(string oFromDocEntry01)
        {
            try
            {
        ...
        /// EnableMenus
        /// </summary>
        private void PS_QM703_EnableMenus()
        {
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
        ...
        /// 모드에 따른 아이템 설정
        /// </summary>
        private void PS_QM703_FormItemEnabled()
        {
            string sQry;
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
        ...
        /// <summary>
        /// PS_QM703_AddMatrixRow
        /// </summary>
        /// <param name="oRow">행 번호</param>
        /// <param name="RowIserted">행 추가 여부</param>
        private void PS_QM703_AddMatrixRow(int oRow, bool RowIserted)
        {
            try
            {
                oForm.Freeze(true);
                ...
                oMat01.AddRow();
                oDS_PS_QM703L.Offset = oRow;
                oDS_PS_QM703L.SetValue("U_LineNum", oRow, Convert.ToString(oRow + 1));
        ...
        /// DocEntry 초기화
        /// </summary>
        private void PS_QM703_FormClear()
        {
            string DocEntry;
            PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
            ...
            try
            {
                DocEntry = dataHelpClass.Get_ReData("AutoKey", "ObjectCode", "ONNM", "'PS_QM703'", "");
        ...
        /// </summary>
        /// <returns></returns>
        private bool PS_QM703_DataValidCheck()
        {
            ...
            try
            {
                ...
                {
                    string sQry = "SELECT CoUNT(*) FROM [@PS_QM700L] WHERE Code = 'ZCpCode' AND U_Code ='" + (oForm.Items.Item("InCpCode").Specific.Value.ToString().Trim()) + "'";
                    oRecordSet.DoQuery(sQry);
        ...
        private bool PS_QM703_LoadPic(string pPictureControlName)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(oDS_PS_QM703H.GetValue("U_WorkNum", 0).ToString().Trim()))
                {
                    throw new Exception();
                }
                ...
                FSO.CopyFile(sFilePath, SaveFolders + "\\" + oDS_PS_QM703H.GetValue("DocEntry", 0).ToString().Trim() + imageFileName);

                sQry = " EXEC [PS_QM703_01] '";
                sQry += oDS_PS_QM703H.GetValue("DocEntry", 0).ToString().Trim() + "'";
                oRecordSet.DoQuery(sQry);
                ...
                else if (pVal.BeforeAction == false)
                {
                    if (pVal.ItemUID == "1")
                    {
                        if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
                        {
                            ...
                            {
                                sQry = "SELECT MAX(DocEntry) AS DocEntry FROM [@PS_QM703H]";
                                oRecordSet.DoQuery(sQry);
                                oForm.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE;
                                PS_QM703_FormItemEnabled();
        ...
        private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                else if (pVal.Before_Action == false)
                {
                    ...
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_QM703L);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (oLastColRow01 > 0)
                {
                    ...
                    else if (pVal.BeforeAction == false)
                    {
                        ...
                        oMat01.FlushToDataSource();
                        oDS_PS_QM703L.RemoveRecord(oDS_PS_QM703L.Size - 1);
                        oMat01.LoadFromDataSource();
                        if (oMat01.RowCount == 0)
                        {
                            PS_QM703_AddMatrixRow(0, false);
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(oDS_PS_QM703L.GetValue("U_spec", oMat01.RowCount - 1).ToString().Trim()))
                            {
                                PS_QM703_AddMatrixRow(oMat01.RowCount, false);
                            }
                        }
                    }
                }
