...
namespace PSH_BOne_AddOn
{
	...
	/// 작번별미청구금액등록
	/// </summary>
	internal class PS_SD057 : PSH_BaseClass
	{
		private string oFormUniqueID;
		private SAPbouiCOM.Matrix oMat;

		private SAPbouiCOM.DBDataSource oDS_PS_SD057L; //등록라인
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_SD057.srf");
				...
				oFormUniqueID = "PS_SD057_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_SD057");
				...
				PS_SD057_CreateItems();
				PS_SD057_ComboBox_Setting();
				PS_SD057_FormResize();
				PS_SD057_LoadCaption();
				PS_SD057_Initial_Setting();
		...
		/// PS_SD057_CreateItems
		/// </summary>
		private void PS_SD057_CreateItems()
		{
			try
			{
				oDS_PS_SD057L = oForm.DataSources.DBDataSources.Item("@PS_USERDS01");
		...
		/// PS_SD057_ComboBox_Setting
		/// </summary>
		private void PS_SD057_ComboBox_Setting()
		{
			string sQry;
		...
		/// PS_SD057_FormResize
		/// </summary>
		private void PS_SD057_FormResize()
		{
			try
			{
				oMat.AutoResizeColumns();
		...
		/// <summary>
		/// PS_SD057_LoadCaption Form의 Mode에 따라 추가, 확인, 갱신 버튼 이름 변경
		/// </summary> 
		private void PS_SD057_LoadCaption()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// PS_SD057_Initial_Setting
		/// </summary>
		private void PS_SD057_Initial_Setting()
		{
			try
			{
		...
		/// PS_SD057_CheckAll
		/// </summary>
		private void PS_SD057_CheckAll()
		{
			string CheckType;
			int loopCount;
			...
			try
			{
				...
				{
					if (oDS_PS_SD057L.GetValue("U_ColReg01", loopCount).ToString().Trim() == "N")
					{
						CheckType = "N";
				...
				for (loopCount = 0; loopCount <= oMat.VisualRowCount - 1; loopCount++)
				{
					oDS_PS_SD057L.Offset = loopCount;
					if (CheckType == "N")
					{
						oDS_PS_SD057L.SetValue("U_ColReg01", loopCount, "Y");
					}
					else
					{
						oDS_PS_SD057L.SetValue("U_ColReg01", loopCount, "N");
					}
				}
		...
		/// <summary>
		/// PS_SD057_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_SD057_FlushToItemValue(string oUID, int oRow, string oCol)
		{
			double Price;     //단가
				...
				switch (oUID)
				{
					case "Mat01":
						if (oCol == "RWorkTime")
						{
							...
							RWorkTime = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColQty01", oRow - 1).ToString().Trim());
							Price = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum01", oRow - 1).ToString().Trim());
							Amount = RWorkTime * Price;
							oDS_PS_SD057L.SetValue("U_ColSum02", oRow - 1, Convert.ToString(Amount));

							for (loopCount = 0; loopCount <= oMat.VisualRowCount - 1; loopCount++)
							{
								TotalAmt += Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum02", loopCount).ToString().Trim());
							}
		...
		/// <summary>
		/// PS_SD057_CheckBeforeSearch 필수입력사항 체크
		/// </summary>
		/// <param name="pItemUID"></param>
		/// <returns></returns>
		private bool PS_SD057_CheckBeforeSearch(string pItemUID)
		{
		...
		/// <summary>
		/// PS_SD057_MTX01 데이터 조회
		/// </summary>
		/// <param name="pItemUID"></param>
		private void PS_SD057_MTX01(string pItemUID)
		{
			int i;
			string sQry;
			string errMessage = string.Empty;
			...
			try
			{
				...
				if (pItemUID == "BtnSearch1")
				{
					...
					{
						if (i + 1 > oDS_PS_SD057L.Size)
						{
							oDS_PS_SD057L.InsertRecord((i));
						}

						oMat.AddRow();
						oDS_PS_SD057L.Offset = i;

						oDS_PS_SD057L.SetValue("U_LineNum", i, Convert.ToString(i + 1));
						oDS_PS_SD057L.SetValue("U_ColReg01", i, oRecordSet.Fields.Item("Check").Value.ToString().Trim());	 //선택
						oDS_PS_SD057L.SetValue("U_ColReg02", i, oRecordSet.Fields.Item("StdYM").Value.ToString().Trim());	 //기준년월
						oDS_PS_SD057L.SetValue("U_ColReg03", i, oRecordSet.Fields.Item("StdCnt").Value.ToString().Trim());	 //기준회차
						oDS_PS_SD057L.SetValue("U_ColReg04", i, oRecordSet.Fields.Item("OrdNum").Value.ToString().Trim());	 //품목코드(작번)
						oDS_PS_SD057L.SetValue("U_ColReg05", i, oRecordSet.Fields.Item("FrgnName").Value.ToString().Trim()); //품목명(작번)
						oDS_PS_SD057L.SetValue("U_ColReg06", i, oRecordSet.Fields.Item("Spec").Value.ToString().Trim());	 //규격(작번)
						oDS_PS_SD057L.SetValue("U_ColReg07", i, oRecordSet.Fields.Item("InOut").Value.ToString().Trim());	 //자체/외주
						oDS_PS_SD057L.SetValue("U_ColSum01", i, oRecordSet.Fields.Item("MatAmt").Value.ToString().Trim());	 //견적자재비
						oDS_PS_SD057L.SetValue("U_ColSum02", i, oRecordSet.Fields.Item("TotAmt").Value.ToString().Trim());	 //누계자재비
						oDS_PS_SD057L.SetValue("U_ColSum04", i, oRecordSet.Fields.Item("PreEA").Value.ToString().Trim());	 //직전미청구금액
						oDS_PS_SD057L.SetValue("U_ColSum03", i, oRecordSet.Fields.Item("ExpAmt").Value.ToString().Trim());	 //미청구금액
						oDS_PS_SD057L.SetValue("U_ColTxt01", i, oRecordSet.Fields.Item("Comment").Value.ToString().Trim());	 //비고
						oDS_PS_SD057L.SetValue("U_ColReg10", i, oRecordSet.Fields.Item("CreateUser").Value.ToString().Trim()); //등록자(사번)
						oDS_PS_SD057L.SetValue("U_ColDt01", i, Convert.ToDateTime(oRecordSet.Fields.Item("CreateDate").Value.ToString().Trim()).ToString("yyyyMMdd")); //등록일자
						oDS_PS_SD057L.SetValue("U_ColReg12", i, oRecordSet.Fields.Item("UpdateUser").Value.ToString().Trim());  //수정자(사번)
						oDS_PS_SD057L.SetValue("U_ColDt02", i, Convert.ToDateTime(oRecordSet.Fields.Item("UpdateDate").Value.ToString().Trim()).ToString("yyyyMMdd")); //수정일자
		...
		/// <summary>
		/// PS_SD057_AddData 데이터 INSERT
		/// </summary>
		/// <returns></returns>
		private bool PS_SD057_AddData()
		{
			bool functionReturnValue = false;
			...
			try
			{
				...
				for (loopCount = 0; loopCount <= oMat.VisualRowCount - 1; loopCount++)
				{
					if (oDS_PS_SD057L.GetValue("U_ColReg01", loopCount).ToString().Trim() == "Y")
					{
						StdYM    = oDS_PS_SD057L.GetValue("U_ColReg02", loopCount).ToString().Trim();
						StdCnt   = oDS_PS_SD057L.GetValue("U_ColReg03", loopCount).ToString().Trim();
						OrdNum   = oDS_PS_SD057L.GetValue("U_ColReg04", loopCount).ToString().Trim();
						FrgnName = oDS_PS_SD057L.GetValue("U_ColReg05", loopCount).ToString().Trim();
						SPEC     = oDS_PS_SD057L.GetValue("U_ColReg06", loopCount).ToString().Trim();
						InOut    = oDS_PS_SD057L.GetValue("U_ColReg07", loopCount).ToString().Trim();
						MatAmt = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum01", loopCount).ToString().Trim());
						TotAmt = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum02", loopCount).ToString().Trim());
						PreEA = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum04", loopCount).ToString().Trim());
						ExpAmt = Convert.ToDouble(oDS_PS_SD057L.GetValue("U_ColSum03", loopCount).ToString().Trim());
						Comment = oDS_PS_SD057L.GetValue("U_ColTxt01", loopCount).ToString().Trim();

						sQry = "  EXEC [PS_SD057_03] ";
						sQry += "'" + StdYM + "',";
						sQry += "'" + StdCnt + "',";
						sQry += "'" + OrdNum + "',";
						sQry += "'" + FrgnName + "',";
		...
		/// <summary>
		/// PS_SD057_DeleteData 기본정보 삭제
		/// </summary>
		private void PS_SD057_DeleteData()
		{
			int loopCount;
			string sQry;
			string errMessage = string.Empty;
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "BtnAdd")
					{
						...
						else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
						{
							...
							PS_SD057_MTX01("BtnSearch2");
							PS_SD057_LoadCaption();
						}
					}
					else if (pVal.ItemUID == "BtnSearch1")
					{
						if (PS_SD057_CheckBeforeSearch(pVal.ItemUID) == false)
						{
							BubbleEvent = false;
							return;
						}
		...
		private void Raise_EVENT_COMBO_SELECT(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				{
					PS_SD057_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
				}
			}
			catch (Exception ex)
			{
			...
			try
			{
				...
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemChanged == true)
					{
						...
						{
							PS_SD057_FlushToItemValue(pVal.ItemUID, 0, "");
						}
					}
				}
				else if (pVal.BeforeAction == false)
		...
		private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					...
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_SD057L);
				}
			}
			catch (Exception ex)
			{
