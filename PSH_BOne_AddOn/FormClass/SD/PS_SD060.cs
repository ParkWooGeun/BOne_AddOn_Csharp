...
namespace PSH_BOne_AddOn
{
	...
	/// 수출신고내역조회
	/// </summary>
	internal class PS_SD060 : PSH_BaseClass
	{
		private string oFormUniqueID;
		private SAPbouiCOM.Grid oGrid;
		private SAPbouiCOM.DataTable oDS_PS_SD060A;
		...
		public override void LoadForm(string oFormDocEntry01)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_SD060.srf");
				...
				oFormUniqueID = "PS_SD060_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_SD060");
		...
		/// PS_SD060_CreateItems
		/// </summary>
		private void PS_SD060_CreateItems()
		{
			try
			{
				oGrid = oForm.Items.Item("Grid01").Specific;
				oForm.DataSources.DataTables.Add("PS_SD060A");
				oGrid.DataTable = oForm.DataSources.DataTables.Item("PS_SD060A");
				oDS_PS_SD060A = oForm.DataSources.DataTables.Item("PS_SD060A");
		...
		/// <summary>
		/// PS_SD060_FlushToItemValue
		/// </summary>
		/// <param name="oUID"></param>
		/// <param name="oRow"></param>
		/// <param name="oCol"></param>
		private void PS_SD060_FlushToItemValue(string oUID, int oRow, string oCol)
		{
		...
		/// PS_SD060_MTX01
		/// </summary>
		private void PS_SD060_MTX01()
		{
			string sQry;
			string errMessage = string.Empty;

			string CardCode; //해외거래처코드
			string FrDt;	 //기간(Fr)
			string ToDt;	 //기간(To)
			...
			try
			{
				...
				sQry = "    EXEC PS_SD060_01 '";
				sQry += CardCode + "','";
				sQry += FrDt + "','";
				sQry += ToDt + "'";

				oGrid.DataTable.Clear();
				oDS_PS_SD060A.ExecuteQuery(sQry);
		...
		/// <summary>
		/// PS_SD060_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_SD060_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string CardCode; //해외거래처코드
			...
			try
			{
				...
				WinTitle = "[PS_SD060] 레포트";
				ReportName = "PS_SD060_01.rpt";
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					...
					else if (pVal.ItemUID == "BtnPrint")
					{
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_SD060_Print_Report01);
		...
		private void Raise_EVENT_VALIDATE(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					...
					{
						PS_SD060_FlushToItemValue(pVal.ItemUID, pVal.Row, pVal.ColUID);
					}
				}
			}
			catch (Exception ex)
		...
		private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
		{
			try
			{
				...
				else if (pVal.BeforeAction == false)
				{
					...
					System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_SD060A);
				}
			}
			catch (Exception ex)
			{
