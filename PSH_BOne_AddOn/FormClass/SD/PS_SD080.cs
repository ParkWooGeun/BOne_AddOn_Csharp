...
namespace PSH_BOne_AddOn
{
	...
	internal class PS_SD080 : PSH_BaseClass
	{
        ...
        /// 모드에 따른 아이템 설정
        /// </summary>
        private void PS_SD080_EnableFormItem()
        {
            try
            {
                ...
                else if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                {
                    oForm.Items.Item("DocNum").Enabled = false;
                    oForm.Items.Item("Btn01").Enabled = false;
                    if (oDS_PS_SD080H.GetValue("U_OkYN", 0).ToString().Trim() == "Y")
                    {
        ...
        private void PS_SD080_AddMatrixRow(int oRow, bool RowIserted)
        {
            try
            {
                if (RowIserted == false)
        ...
        private void PS_SD080_FlushToItemValue(string oUID, int oRow, string oCol)
        {
            string sQry;
            SAPbobsCOM.Recordset oRecordSet01 = PSH_Globals.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            ...
            try
            {
                ...
                {
                    case "CntcCode":
                        sQry = "Select lastName + firstName From OHEM Where U_MSTCOD = '" + oDS_PS_SD080H.GetValue("U_CntcCode", 0).ToString().Trim() + "'";
                        oRecordSet01.DoQuery(sQry);

                        oDS_PS_SD080H.SetValue("U_CntcName", 0, oRecordSet01.Fields.Item(0).Value.ToString().Trim());
                        break;
                }
            }
            catch(Exception ex)
        ...
        private bool PS_SD080_DeleteHeaderSpaceLine()
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(oDS_PS_SD080H.GetValue("U_BPLId", 0)))
                {
                    errMessage = "사업장은 필수사항입니다. 확인하세요.";
                    throw new Exception();
                }
                else if (string.IsNullOrEmpty(oDS_PS_SD080H.GetValue("U_CntcCode", 0)))
                {
                    errMessage = "작성자는 필수사항입니다. 확인하세요.";
                    throw new Exception();
                }
                else if (string.IsNullOrEmpty(oDS_PS_SD080H.GetValue("U_DocDate", 0)))
                {
                    errMessage = "요청일자는 필수사항입니다. 확인하세요.";
                    throw new Exception();
                }
        ...
        private bool PS_SD080_DeleteMatrixSpaceLine()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= oMat01.VisualRowCount - 1; i++)
                {
                    ...
                    {
                        errMessage = "초과필요금액은 0 보다 커야 합니다. " + (i + 1) + "행을 확인하세요.";
                        throw new Exception();
                    }
                }
        ...
        /// Mat01에 마지막라인(빈라인) 삭제
        /// </summary>
        private void PS_SD080_DeleteEmptyRow()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= oMat01.VisualRowCount - 1; i++)
                {
                    if (string.IsNullOrEmpty(oDS_PS_SD080L.GetValue("U_CGNo", i).ToString().Trim()))
                    {
        ...
        private void PS_SD080_PrintReport01()
        {
            ...
            try
            {
                ...
                {
                    new PSH_DataPackClass("@DocNum", docNum)
                };
        ...
        private void Raise_EVENT_ITEM_PRESSED(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                else if (pVal.BeforeAction == false)
                {
                    ...
                    {
                        PS_SD081 tempForm = new PS_SD081();
                        tempForm.LoadForm(oForm, pVal.Row);
                        BubbleEvent = false;
                    }
                    else if (pVal.ItemUID == "Btn02")
                    {
                        System.Threading.Thread thread = new System.Threading.Thread(PS_SD080_PrintReport01);
            ...
            try
            {
                ...
                else if (pVal.Before_Action == false)
                {
                    if (pVal.ItemChanged == true)
                    {
                        if (pVal.ItemUID == "CntcCode")
                        {
                            PS_SD080_FlushToItemValue(pVal.ItemUID, 0, "");
                        }
                    }
                }
            }
        ...
        private void Raise_EVENT_FORM_UNLOAD(string FormUID, ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                else if (pVal.Before_Action == false)
                {
                    ...
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_SD080H);
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(oDS_PS_SD080L);
                }
            }
            catch (Exception ex)
            {
