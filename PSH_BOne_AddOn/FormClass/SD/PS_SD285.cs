...
namespace PSH_BOne_AddOn
{
	...
	/// 타계정출고현황(샘플,감모손)
	/// </summary>
	internal class PS_SD285 : PSH_BaseClass
	{
		...
		/// LoadForm
		/// </summary>
		/// <param name="oFormDocEntry"></param>PS_SD285_SetComboBox
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_SD285.srf");
				...
				oFormUniqueID = "PS_SD285_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_SD285");
				...
				oForm.Freeze(true);
				PS_SD285_CreateItems();
				PS_SD285_SetComboBox();
				PS_SD285_Initialize();
			}
			catch (Exception ex)
			{
		...
		/// PS_SD285_CreateItems
		/// </summary>
		private void PS_SD285_CreateItems()
		{
			try
			{
				oForm.Freeze(true);
				...
				oForm.DataSources.UserDataSources.Add("IssueType", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 100);
				...
				oForm.Items.Item("IssueType").Specific.DataBind.SetBound(true, "", "IssueType");
		...
		/// PS_SD285_SetComboBox
		/// </summary>
		private void PS_SD285_SetComboBox()
		{
			...
			try
			{
				...
				dataHelpClass.Set_ComboList(oForm.Items.Item("IssueType").Specific, "SELECT Code, Name FROM [@PSH_ISSUETYPE] ORDER BY Code", "", false, false);
				oForm.Items.Item("IssueType").Specific.Select(1, SAPbouiCOM.BoSearchKey.psk_Index);
			}
			catch (Exception ex)
			{
				PSH_Globals.SBO_Application.StatusBar.SetText(System.Reflection.MethodBase.GetCurrentMethod().Name + "_Error : " + ex.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
		...
		/// PS_SD285_Initialize
		/// </summary>
		private void PS_SD285_Initialize()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
		...
		/// <summary>
		/// PS_SD285_CheckDataValid
		/// </summary>
		/// <returns></returns>
		private bool PS_SD285_CheckDataValid()
		{
			...
			catch (Exception ex)
			{
				if (ErrNum == 1)
				{
					PSH_Globals.SBO_Application.SetStatusBarMessage("조회일자 Fr은 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
					oForm.Items.Item("DocDateFr").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
				}
				else if (ErrNum == 2)
				{
					PSH_Globals.SBO_Application.SetStatusBarMessage("조회일자 To는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
		...
		/// <summary>
		/// PS_SD285_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_SD285_Print_Report01()
		{
			string sQry;
			string WinTitle;
			string ReportName;
			...
			try
			{
				...
				WinTitle = "타계정출고현황(샘플,감모손) [PS_SD285]";
				ReportName = "PS_SD285_01.rpt";
				...
				dataPackParameter.Add(new PSH_DataPackClass("@IssueTyp", IssueType));
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Btn01")
					{
						...
						{
							if (PS_SD285_CheckDataValid() == false)
							{
								BubbleEvent = false;
								return;
							}
							...
							{
								System.Threading.Thread thread = new System.Threading.Thread(PS_SD285_Print_Report01);
