...
namespace PSH_BOne_AddOn
{
	...
	/// 판매현황(멀티)
	/// </summary>
	internal class PS_SD400 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry01)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_SD400.srf");
				...
				oFormUniqueID = "PS_SD400_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_SD400");
				...
				oForm.Freeze(true);
				PS_SD400_CreateItems();
				PS_SD400_ComboBox_Setting();
				PS_SD400_Initial_Setting();
			}
			catch (Exception ex)
			{
		...
		/// PS_SD400_CreateItems
		/// </summary>
		private void PS_SD400_CreateItems()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// PS_SD400_ComboBox_Setting
		/// </summary>
		public void PS_SD400_ComboBox_Setting()
		{
		...
		/// PS_SD400_Initial_Setting
		/// </summary>
		public void PS_SD400_Initial_Setting()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Btn01")
					{
						...
						{
							if (PS_SD400_DataValidCheck() == false)
							{
								BubbleEvent = false;
								return;
							}
						...
						if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE)
						{
							System.Threading.Thread thread = new System.Threading.Thread(PS_SD400_Print_Report01);
		...
		/// <summary>
		/// PS_SD400_DataValidCheck
		/// </summary>
		/// <returns></returns>
		public bool PS_SD400_DataValidCheck()
		{
			...
			catch (Exception ex)
			{
				if (ErrNum == 1)
				{
					PSH_Globals.SBO_Application.SetStatusBarMessage("시작기준일자는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
					oForm.Items.Item("DocDateFr").Click(SAPbouiCOM.BoCellClickType.ct_Regular);
				}
				else if (ErrNum == 2)
				{
					PSH_Globals.SBO_Application.SetStatusBarMessage("종료기준일자는 필수입니다.", SAPbouiCOM.BoMessageTime.bmt_Short, true);
		...
		/// <summary>
		/// PS_SD400_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_SD400_Print_Report01()
		{
			...
			try
			{
				...
				{
					WinTitle = "[PS_SD400_01] 판매현황 (품목별)";
					ReportName = "PS_SD400_01.rpt";
				}
				else if (oForm.DataSources.UserDataSources.Item("Rad01").Value.ToString().Trim() == "B")
				{
					WinTitle = "[PS_SD400_02] 판매현황 (거래처별)";
					ReportName = "PS_SD400_02.rpt";
				}
				...
				dataPackParameter.Add(new PSH_DataPackClass("@BeginDay", BeginDay));
