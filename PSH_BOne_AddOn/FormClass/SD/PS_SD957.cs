...
namespace PSH_BOne_AddOn
{
	...
	/// 거래처별 세분화별 수주실적(관계3사)
	/// </summary>
	internal class PS_SD957 : PSH_BaseClass
	{
		...
		public override void LoadForm(string oFormDocEntry)
		{
			...
			try
			{
				oXmlDoc.load(PSH_Globals.SP_Path + "\\" + PSH_Globals.Screen + "\\PS_SD957.srf");
				...
				oFormUniqueID = "PS_SD957_" + SubMain.Get_TotalFormsCount();
				SubMain.Add_Forms(this, oFormUniqueID, "PS_SD957");
				...
				oForm.Freeze(true);
				PS_SD957_CreateItems();
				PS_SD957_SetComboBox();
				PS_SD957_Initialize();
			}
			catch (Exception ex)
			{
		...
		/// PS_SD957_CreateItems
		/// </summary>
		private void PS_SD957_CreateItems()
		{
			try
			{
				oForm.Freeze(true);
		...
		/// PS_SD957_SetComboBox
		/// </summary>
		private void PS_SD957_SetComboBox()
		{
		...
		/// PS_SD957_Initialize
		/// </summary>
		private void PS_SD957_Initialize()
		{
			PSH_DataHelpClass dataHelpClass = new PSH_DataHelpClass();
		...
		/// <summary>
		/// PS_SD957_CheckDataValid
		/// </summary>
		/// <returns></returns>
		private bool PS_SD957_CheckDataValid()
		{
			...
			try
			{
				...
				else if (string.IsNullOrEmpty(oForm.Items.Item("DocDateFr").Specific.value.ToString().Trim()))
				{
					ErrNum = 2;
					throw new Exception();
				}
		...
		/// <summary>
		/// PS_SD957_Print_Report01
		/// </summary>
		[STAThread]
		private void PS_SD957_Print_Report01()
		{
			string WinTitle;
			string ReportName;
			string BPLId;
			...
			try
			{
				...
				WinTitle = "[PS_SD957] 수주/견적 비교 현황(건별)";
				ReportName = "PS_SD957.rpt";
			...
			try
			{
				if (pVal.BeforeAction == true)
				{
					if (pVal.ItemUID == "Btn01")
					{
						...
						{
							if (PS_SD957_CheckDataValid() == false)
							{
								BubbleEvent = false;
								return;
							}
							...
							{
								System.Threading.Thread thread = new System.Threading.Thread(PS_SD957_Print_Report01);
